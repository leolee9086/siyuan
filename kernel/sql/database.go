...
func init() {
	...
	sql.Register("sqlite3_extended", &sqlite3.SQLiteDriver{
		ConnectHook: func(conn *sqlite3.SQLiteConn) error {
			...
			if err := conn.RegisterFunc("ciede2000_rgb", func(r1, g1, b1, r2, g2, b2 int64) float64 {
...
func batchUpdatePath(tx *sql.Tx, tree *parse.Tree, context map[string]any) (err error) {
	...
	// external content FTS 必须在 blocks 仍保留旧值时更新索引列，才能正确删除旧 token。
	stmt := "UPDATE blocks_fts SET ial = ? WHERE rowid IN (SELECT rowid FROM blocks WHERE root_id = ?)"
	if err = execStmtTx(tx, stmt, ialContent, tree.ID); err != nil {
		return
	}
