...
func TestControlConcurrencyTracksOnlyStaticAPIRoutes(t *testing.T) {
	requestingLock.Lock()
	originalRequesting := requesting
	requesting = map[string]*sync.Mutex{}
	requestingLock.Unlock()
	t.Cleanup(func() {
		requestingLock.Lock()
		requesting = originalRequesting
		requestingLock.Unlock()
	})

	engine := gin.New()
	engine.Use(ControlConcurrency)
	engine.GET("/api/write", func(c *gin.Context) { c.Status(http.StatusNoContent) })
	engine.GET("/api/items/:id", func(c *gin.Context) { c.Status(http.StatusNoContent) })

	for _, requestPath := range []string{
		"/api/write",
		"/api/items/first",
		"/api/missing/high-cardinality-value",
	...
	if len(requesting) != 1 || requesting["/api/write"] == nil {
		t.Fatalf("concurrency map = %#v, want only the static API route", requesting)
	}
}
