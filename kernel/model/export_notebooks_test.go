...
func TestExportFunctionsAcceptEmptyIDs(t *testing.T) {
	if got := ExportSYs(nil); got != "" {
		t.Fatalf("ExportSYs(nil) = %q, want empty path", got)
	}
	if name, got := ExportPandocConvertZip(nil, "", ".md"); name != "" || got != "" {
		t.Fatalf("ExportPandocConvertZip(nil) = (%q, %q), want empty values", name, got)
	}
}
