...
var (
	errContextCannotBeCompacted = errors.New("agent context cannot be compacted enough")
	errCompactionSummaryEmpty   = errors.New("agent compaction summary is empty")
)
...
func checkpointMessagesAfterCompaction(entries []SessionEntry, coveredEntryCount int, currentTail []AgentMessage) []AgentMessage {
	if coveredEntryCount < 0 || coveredEntryCount > len(entries) {
		return append([]AgentMessage(nil), currentTail...)
	}
