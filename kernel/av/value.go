...
type ValueRollup struct {
	...
	// 缓存相关字段（不序列化到JSON）
	cachedContents   []*Value     `json:"-"` // 缓存的计算结果
	cacheFingerprint string       `json:"-"` // 数据指纹，用于判断数据是否变化
	cacheTimestamp   int64        `json:"-"` // 缓存时间戳（毫秒）
	cacheMutex       sync.RWMutex `json:"-"` // 并发安全锁
}

// calculateFingerprint 计算数据指纹，用于判断数据是否变化。
// 指纹同时包含关联目标值与 EligibleItemIDs 集合，避免目标单元格更新或过滤集合变化后继续复用旧汇总结果。
func (r *ValueRollup) calculateFingerprint(keyValues []*KeyValues, destKey *Key, relationVal *Value, calc *RollupCalc, context *RollupRenderContext) string {
	...
	if nil != context && nil != context.EligibleItemIDs {
		for blockID := range context.EligibleItemIDs {
			fingerprintData += "|eligible=" + blockID
		}
	}
	for _, blockID := range relationVal.Relation.BlockIDs {
		fingerprintData += "|" + blockID + "="
		if destVal := GetValue(keyValues, destKey.ID, blockID); nil != destVal {
...
// 缓存有效条件：1) 指纹匹配 2) 未过期（5分钟内）
func (r *ValueRollup) isCacheValid(fingerprint string) bool {
	if r.cacheFingerprint != fingerprint {
		return false
	}

	// 检查缓存是否过期（5分钟 = 300000毫秒）
	const cacheExpireTime = 300000
	now := time.Now().UnixMilli()
	if now-r.cacheTimestamp > cacheExpireTime {
		return false
	}

	return r.cachedContents != nil
}
...
func (r *ValueRollup) BuildContents(keyValues []*KeyValues, destKey *Key, relationVal *Value, calc *RollupCalc, context *RollupRenderContext) {
	// 计算数据指纹
	fingerprint := r.calculateFingerprint(keyValues, destKey, relationVal, calc, context)
	...
	// 检查缓存是否有效
	if r.isCacheValid(fingerprint) {
		// 缓存有效，深拷贝缓存结果到Contents，避免外部修改影响缓存
		r.Contents = make([]*Value, len(r.cachedContents))
		for i, v := range r.cachedContents {
			r.Contents[i] = v.Clone()
		}
		return
	}
	...
	// 更新缓存
	// 深拷贝Contents到cachedContents，避免外部修改影响缓存
	r.cachedContents = make([]*Value, len(r.Contents))
	for i, v := range r.Contents {
		r.cachedContents[i] = v.Clone()
	}
	r.cacheFingerprint = fingerprint
