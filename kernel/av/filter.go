...
func collectLeafColumnIndexes(nodes []*ViewFilter, fields []Field, colIndexByColumn map[string]int) {
	...
	collectLeafColumns(nodes, columns)
	if 1 > len(columns) {
		return
	}
	...
	if 10 < len(fields) {
		...
		for column := range columns {
			if index, exists := keyIndexMap[column]; exists {
				colIndexByColumn[column] = index
			}
		}
		return
	}
	...
	for column := range columns {
		for i, field := range fields {
			if field.GetID() == column {
				colIndexByColumn[column] = i
				break
			}
		}
	}
...
func collectLeafColumns(nodes []*ViewFilter, columns map[string]struct{}) {
	for _, node := range nodes {
		if nil == node {
			continue
		}
		if node.IsGroup() {
			collectLeafColumns(node.Filters, columns)
			continue
		}
		if "" == node.Column {
			continue
