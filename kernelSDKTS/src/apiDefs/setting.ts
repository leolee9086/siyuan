...
/** Basic认证账户 Schema */
const Basic认证账户Schema = z.object({
...
/** Basic认证配置 Schema */
const Basic认证配置Schema = z.object({
    enable: z.boolean().describe('是否启用 Basic 认证。'),
    accounts: z.array(Basic认证账户Schema).describe('Basic 认证账户列表。'),
});

/** 发布配置 Schema */
const 发布配置Schema = z.object({
    enable: z.boolean().describe('是否启用发布服务。'),
    port: z.number().int().describe('发布服务配置的端口号。'),
    auth: Basic认证配置Schema.describe('Basic 认证配置。'),
});

/** OpenAI 配置 Schema */
const OpenAI配置Schema = z.object({
    apiKey: z.string().optional().describe('OpenAI API 密钥。'),
    apiTimeout: z.number().int().optional().describe('OpenAI API 请求超时时间 (秒)。'),
    apiProxy: z.string().optional().describe('OpenAI API 代理地址。'),
    apiModel: z.string().optional().describe('OpenAI API 模型名称。'),
    apiMaxTokens: z.number().int().optional().describe('OpenAI API 最大 token 数。'),
    apiTemperature: z.number().optional().describe('OpenAI API 温度系数。'),
    apiMaxContexts: z.number().int().optional().describe('OpenAI API 最大上下文数量。'),
...
export const settingApiDefs = [
    ...
    },
    {
        ...
        zodRequestSchema: z.object({
            enable: z.boolean().describe('是否启用发布服务。'),
            port: z.number().int().describe('发布服务监听的端口号。'),
            auth: z.object({
                enable: z.boolean().describe('是否启用 Basic 认证。'),
        ...
        zodResponseSchema: 创建响应Schema(
            z.object({
                port: z.number().describe('发布服务实际监听的端口号。'),
                publish: 发布配置Schema.describe('详细的发布配置项。'),
            }).nullable()
        ),
    },
    ...
    },
    {
        ...
        zodRequestSchema: z.object({
            ...
            maxOpenTabCount: z.number().int().optional().describe('编辑器最大可打开的页签数量。'),
    ...
    },
    {
        ...
        unavailableIfReadonly: true,
        zodRequestSchema: z.object({
            newCardLimit: z.number().int().optional().describe('每日新建卡片上限。'),
            reviewCardLimit: z.number().int().optional().describe('每日复习卡片上限。'),
            requestRetention: z.number().optional().describe('期望记忆留存率 (FSRS)。'),
            maximumInterval: z.number().int().optional().describe('最大复习间隔天数 (FSRS)。'),
            easyBonus: z.number().optional().describe('简单奖励系数 (FSRS)。'),
            hardInterval: z.number().optional().describe('困难间隔系数 (FSRS)。'),
            lapseInterval: z.number().optional().describe('失误间隔系数 (FSRS)。'),
            againInterval: z.number().optional().describe('重来间隔系数 (FSRS)。'),
        }),
        zodResponseSchema: 创建响应Schema(
            z.object({
    ...
    },
    {
        ...
        zodRequestSchema: z.object({
            ...
            limit: z.number().int().optional().describe('搜索结果数量限制，最小32。'),
...
export type SettingApiDefs = typeof settingApiDefs;
