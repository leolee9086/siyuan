...
/** 单个事务的定义 (请求用) */
const 事务Schema请求 = z.object({
    timestamp: z.number().int().describe('事务时间戳，通常由前端生成或后端使用 reqId 覆盖（毫秒级）。'),
    doOperations: z.array(操作Schema请求).min(1).describe('要执行的操作列表，至少包含一个操作。'),
    undoOperations: z.array(操作Schema请求).optional().describe('可选的撤销操作列表。'),
}).describe('单个事务的定义。');

// ====== 响应用的 Schema ======

/** 单个操作的定义 (响应用，包含 retData) */
const 操作Schema响应 = z.object({
...
/** 单个事务的定义 (响应用) */
const 事务Schema响应 = z.object({
    timestamp: z.number().int().describe('事务时间戳，与请求中的 reqId 一致（毫秒级）。'),
    doOperations: z.array(操作Schema响应).min(1).describe('已执行的操作列表，其中 retData 可能包含操作结果。'),
    undoOperations: z.array(操作Schema响应).optional().describe('对应的撤销操作列表（如果请求中提供）。'),
}).describe('单个事务的定义（响应）。');
...
export const transactionsApiDefs = [
    {
        ...
        zodRequestSchema: z.object({
            transactions: z.array(事务Schema请求).min(1).describe('一个或多个事务对象的数组，至少包含一个事务。'),
...
export type TransactionsApiDefs = typeof transactionsApiDefs;
