...
export const fileApiDefs = [
    {
        ...
        zodRequestSchema: z.object({
            path: z.string().describe('要获取内容的文件路径 (相对于工作空间根目录)。'),
        }),
    ...
    },
    {
        ...
        zodRequestSchema: z.object({
            path: z.string().describe('目标文件或目录在工作空间内的相对路径。文件名需要符合规范，否则请求失败。'),
            isDir: z.boolean().describe('是否创建目录。如果为 true，则创建目录；如果为 false 或未提供，则上传文件。'),
            modTime: z.string().optional().describe('可选的文件修改时间戳 (毫秒级字符串)。如果提供，则设置文件或目录的修改时间。'),
    ...
    },
    {
        ...
        unavailableIfReadonly: false,
        zodRequestSchema: z.object({
            path: z.string().describe('要读取的目录路径 (相对于工作空间根目录)。'),
        }),
    ...
    },
    {
        ...
        unavailableIfReadonly: false,
        zodRequestSchema: z.object({
            path: z.string().describe('原始文件路径 (通常包含期望的文件名和扩展名)。'),
        }),
        zodResponseSchema: 创建响应Schema(z.object({
...
export type FileApiDefs = typeof fileApiDefs;
