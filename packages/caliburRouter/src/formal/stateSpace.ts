...
export type FormalStateSpace =
    | { readonly kind: "unit"; readonly value: FormalUnit }
    | { readonly kind: "domain"; readonly domain: FormalDomain }
    | { readonly kind: "object"; readonly properties: Readonly<Record<string, FormalStateSpace>> }
    | { readonly kind: "union"; readonly branches: readonly FormalStateSpace[] };

type PropertyPath = readonly string[];

export function formalUnit(value: FormalUnit): FormalStateSpace {
...
export function formalDomain(domain: FormalDomain): FormalStateSpace {
    return { kind: "domain", domain };
}

export function formalObject(properties: Readonly<Record<string, FormalStateSpace>>): FormalStateSpace {
    return { kind: "object", properties };
}

export function formalUnion(branches: readonly FormalStateSpace[]): FormalStateSpace {
...
function sameUnit(left: FormalUnit, right: FormalUnit): boolean {
    return Object.is(left, right);
}

function unitInDomain(value: FormalUnit, domain: FormalDomain): boolean {
    return domain === "number"
        ? typeof value === "number" && Number.isFinite(value)
        : typeof value === domain;
}

function flattenBranches(space: FormalStateSpace): readonly FormalStateSpace[] {
    return space.kind === "union" ? space.branches.flatMap(flattenBranches) : [space];
}

function isSubsetOfSingle(left: FormalStateSpace, right: FormalStateSpace): boolean {
    if (left.kind === "union") {
...
export function formalIsSubset(left: FormalStateSpace, right: FormalStateSpace): boolean {
    return right.kind === "union"
        ? formalCovers(left, right.branches)
        : isSubsetOfSingle(left, right);
}

function intersectSingle(left: FormalStateSpace, right: FormalStateSpace): FormalStateSpace | null {
    ...
    const properties: Record<string, FormalStateSpace> = { ...left.properties };
...
export function formalIntersection(
    left: FormalStateSpace,
    right: FormalStateSpace,
): FormalStateSpace | null {
    const intersections: FormalStateSpace[] = [];
...
export function formalOverlaps(left: FormalStateSpace, right: FormalStateSpace): boolean {
    return formalIntersection(left, right) !== null;
}

function collectLeafPaths(space: FormalStateSpace, prefix: PropertyPath, result: Map<string, PropertyPath>): void {
...
function getAtPath(space: FormalStateSpace, path: PropertyPath, depth = 0): FormalStateSpace | null {
    const values: FormalStateSpace[] = [];
    for (const branch of flattenBranches(space)) {
        if (branch.kind !== "object") {
            return null;
        }
...
function finiteUnits(space: FormalStateSpace): readonly FormalUnit[] | null {
    const values: FormalUnit[] = [];
    for (const branch of flattenBranches(space)) {
        if (branch.kind === "unit") {
...
function constraintAtPath(path: PropertyPath, value: FormalUnit): FormalStateSpace {
...
function proveBranchCovered(
    universe: FormalStateSpace,
    patterns: readonly FormalStateSpace[],
): boolean {
    if (patterns.some((pattern) => isSubsetOfSingle(universe, pattern))) {
        return true;
    }

    const constrainedPaths = new Map<string, PropertyPath>();
    ...
    const prove = (region: FormalStateSpace, partitionIndex: number): boolean => {
        ...
        for (const value of partitions[partitionIndex].values) {
            const intersection = formalIntersection(
                region,
                constraintAtPath(partitions[partitionIndex].path, value),
            );
...
export function describeFormalStateSpace(space: FormalStateSpace): string {
    switch (space.kind) {
        case "unit":
