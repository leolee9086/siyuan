 ...
 */
export type 断言子分发器全集合法<子全集, 剩余集, 模式定义, 严格模式 extends boolean = false> = 严格模式 extends true ? 是子集类型<子全集, 模式定义> extends false ? [
...
] : 子全集 : 交集非空<子全集, 模式定义> extends false ? [
    "❌ 类型错误：子分发器全集与当前模式无交集",
    {
        子分发器全集: 子全集;
        当前模式: 模式定义;
 ...
 */
export type 合并类型<全集, 模式> = 全集 extends object ? 模式 extends object ? {
    [K in keyof 全集]: K extends keyof 模式 ? 合并类型<全集[K], 模式[K]> : 全集[K];
} : 模式 : 模式;
/**
 * 匹配器构建器接口
 * 提供链式API用于定义模式分发规则
