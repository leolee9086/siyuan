import { formalDomain, formalObject, formalUnion, formalUnit, } from "./stateSpace.js";
const annotationKeys = new Set(["$schema", "$id", "$comment", "title", "description", "examples"]);
function isRecord(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
function isFormalUnit(value) {
    return value === null ||
        typeof value === "string" ||
        typeof value === "boolean" ||
        (typeof value === "number" && Number.isFinite(value));
}
function assertOnlyKeys(schema, structuralKeys, path) {
    const allowed = new Set([...annotationKeys, ...structuralKeys]);
    const unsupported = Object.keys(schema).filter((key) => !allowed.has(key));
    if (unsupported.length > 0) {
        throw new TypeError(`calibur-router: ${path} 包含不可形式化的 JSON Schema 关键字: ${unsupported.join(", ")}。`);
    }
}
function parseConst(schema, path) {
...
function parseEnum(schema, path) {
...
function parseObject(schema, path) {
...
export function parseFormalJsonSchema(schema, path = "schema") {
    if (!isRecord(schema)) {
    ...
    if ("anyOf" in schema) {
        ...
        return formalUnion(schema.anyOf.map((branch, index) => parseFormalJsonSchema(branch, `${path}.anyOf[${index}]`)));
    }
    switch (schema.type) {
        case "string":
