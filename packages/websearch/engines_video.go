	...
	return newHTMLScraperEngine(htmlScraperConfig{
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if title == "" { continue }
				pos++; results = append(results, SearchResult{Title: title, URL: "https://vimeo.com" + m[1], Snippet: "", Engine: "vimeo", Position: pos, Category: "video"})
			}
			return results, nil
		},
	})(config)
...
func newDailymotion(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct{ List []struct{ Title, URL, Description string; CreatedTime int64 `json:"created_time"`; Duration int } `json:"list"` }
			if err := json.Unmarshal(data, &resp); err != nil || resp.List == nil { return nil, nil }
			var results []SearchResult
			for i, item := range resp.List {
				if i >= max || item.Title == "" || item.URL == "" { break }
...
func newRumble(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "rumble",
		BuildURL: func(q string, opts SearchOptions) string { return "https://rumble.com/search/video?q=" + url.QueryEscape(q) },
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult; pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if title == "" { continue }
				href, views := m[1], m[3]
				if !strings.HasPrefix(href, "http") { href = "https://rumble.com" + href }
				pos++; snippet := "Rumble video"
				if views != "" { snippet = views + " views" }
...
func (e *odyseeEngine) Search(query string, opts SearchOptions, headers map[string]string) ([]SearchResult, error) {
	...
	var data struct{ Result []struct{ Name, Title, Description, ClaimID, ChannelName string; Duration int; ViewCount, CreationTimestamp int64 } `json:"result"` }
	if err := json.Unmarshal([]byte(resp), &data); err != nil { return nil, nil }
	var results []SearchResult
	for i, item := range data.Result {
		if i >= opts.NumResults || (item.Title == "" && item.Name == "") { break }
		title := item.Title; if title == "" { title = item.Name }
		var parts []string
		if item.ChannelName != "" { parts = append(parts, item.ChannelName) }
		if item.ViewCount > 0 { parts = append(parts, formatViews(item.ViewCount)+" views") }
		if item.Duration > 0 { parts = append(parts, formatDuration(item.Duration)) }
		url := "https://odysee.com/" + item.ChannelName + "/" + item.Name
		if item.Name == "" { url = "https://odysee.com/" + item.ClaimID }
		snippet := strings.Join(parts, " · ")
		if snippet == "" { snippet = truncate(item.Description, 300) }
...
func (e *bitchuteEngine) Search(query string, opts SearchOptions, headers map[string]string) ([]SearchResult, error) {
	...
	if err != nil || status < 200 || status >= 400 { return nil, nil }
	var data struct{ Videos []struct{ VideoID, VideoName, Description, Duration, DatePublished string; ViewCount int64; Channel *struct{ ChannelName string } `json:"channel"` } `json:"videos"` }
	if err := json.Unmarshal([]byte(resp), &data); err != nil { return nil, nil }
	var results []SearchResult
	for i, item := range data.Videos {
		if i >= opts.NumResults || item.VideoID == "" || item.VideoName == "" { break }
		author := ""; if item.Channel != nil { author = item.Channel.ChannelName }
		var parts []string
		if author != "" { parts = append(parts, author) }
		if item.Duration != "" { parts = append(parts, item.Duration) }
		if item.ViewCount > 0 { parts = append(parts, fmt.Sprintf("%d views", item.ViewCount)) }
		snippet := ""; if len(parts) > 0 { snippet = "[" + strings.Join(parts, " · ") + "]" + item.Description } else { snippet = item.Description; if snippet == "" { snippet = "BitChute video" } }
		var ts int64
		if item.DatePublished != "" { if t, err := time.Parse(time.RFC3339, item.DatePublished); err == nil { ts = t.UnixMilli() } }
...
func newAcfun(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "acfun",
		BuildURL: func(q string, opts SearchOptions) string { return "https://www.acfun.cn/search?keyword=" + url.QueryEscape(q) + "&pCursor=1" },
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult; pos := 0
			pipeRe := regexp.MustCompile(`bigPipe\.onPageletArrive\((\{[\s\S]*?\})\);`)
			for _, pm := range pipeRe.FindAllStringSubmatch(body, -1) {
				if len(results) >= max { break }
				var pagelet struct{ HTML string `json:"html"` }
				if err := json.Unmarshal([]byte(pm[1]), &pagelet); err != nil || pagelet.HTML == "" { continue }
				...
				for _, vm := range videoRe.FindAllStringSubmatch(pagelet.HTML, -1) {
					...
					if err := json.Unmarshal([]byte(vm[1]), &expo); err != nil || expo.ContentID == "" || expo.Title == "" { continue }
					dur := regexp.MustCompile(`duration[^>]*>([^<]+)<`).FindStringSubmatch(vm[2])
					tm := regexp.MustCompile(`create-time[^>]*>([^<]+)<`).FindStringSubmatch(vm[2])
					parts := []string{}
					if len(dur) > 1 { parts = append(parts, strings.TrimSpace(dur[1])) }
					if len(tm) > 1 { parts = append(parts, strings.TrimSpace(tm[1])) }
					pos++; results = append(results, SearchResult{Title: expo.Title, URL: "https://www.acfun.cn/v/ac" + expo.ContentID, Snippet: strings.Join(parts, " · "), Engine: "acfun", Position: pos, Category: "video"})
				}
			}
			return results, nil
		},
	...
	return newHTMLScraperEngine(htmlScraperConfig{
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				if len(results) >= max { break }
				if m[2] == "" || m[1] == "" { continue }
				href := m[1]; if strings.HasPrefix(href, "//") { href = "https:" + href } else if strings.HasPrefix(href, "/") { href = "https:" + href }
				pos++; results = append(results, SearchResult{Title: m[2], URL: href, Snippet: "iQiyi video", Engine: "iqiyi", Position: pos, Category: "video"})
			}
			if len(results) == 0 {
...
func newNiconico(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "nicovideo",
		BuildURL: func(q string, opts SearchOptions) string { return "https://www.nicovideo.jp/search/" + url.QueryEscape(q) },
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult; pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if title == "" || m[1] == "" { continue }
				vidID := regexp.MustCompile(`/watch/([a-z0-9]+)`).FindStringSubmatch(m[1])
				id := ""; if len(vidID) > 1 { id = vidID[1] } else { ps := strings.Split(m[1], "/"); id = ps[len(ps)-1] }
				pos++; results = append(results, SearchResult{Title: title + " (" + vidLen + ")", URL: "https://www.nicovideo.jp/watch/" + id, Snippet: "Niconico · " + vidLen, Engine: "nicovideo", Position: pos, Category: "video"})
			}
			if len(results) == 0 {
				...
				for _, m := range fbRe.FindAllStringSubmatch(body, -1) {
					...
					if title == "" { continue }
					pos++; results = append(results, SearchResult{Title: title, URL: "https://www.nicovideo.jp/watch/" + m[1], Snippet: "Niconico video", Engine: "nicovideo", Position: pos, Category: "video"})
				}
			}
			return results, nil
		},
...
func newPeerTube(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct{ Data []struct{ URL, Name, Description, PublishedAt string; Duration, Views int; Channel *struct{ DisplayName, Name, Host string } } `json:"data"` }
			if err := json.Unmarshal(data, &resp); err != nil { return nil, nil }
			var results []SearchResult
			for i, v := range resp.Data {
				if i >= max || v.URL == "" || v.Name == "" { break }
				cn := ""; host := ""
				if v.Channel != nil { cn = v.Channel.DisplayName; if cn == "" { cn = v.Channel.Name }; host = v.Channel.Host }
				var parts []string
				if cn != "" { parts = append(parts, cn) }
				if host != "" { parts = append(parts, "@"+host) }
				if v.Duration > 0 { parts = append(parts, formatDuration(v.Duration)) }
				if v.Views > 0 { parts = append(parts, fmt.Sprintf("%d views", v.Views)) }
				snippet := strings.Join(parts, " · "); if snippet == "" { snippet = "PeerTube video" }
				var ts int64; if v.PublishedAt != "" { if t, err := time.Parse(time.RFC3339, v.PublishedAt); err == nil { ts = t.UnixMilli() } }
...
func newSepiaSearch(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct{ Data []struct{ UUID, Name, URL, Description, CreatedAt string; Duration, Views int; Channel *struct{ Name string } } `json:"data"` }
			if err := json.Unmarshal(data, &resp); err != nil { return nil, nil }
			var results []SearchResult
			for i, v := range resp.Data {
				if i >= max || v.Name == "" || (v.UUID == "" && v.URL == "") { break }
				cn := ""; if v.Channel != nil { cn = v.Channel.Name }
				parts := []string{}
				if cn != "" { parts = append(parts, cn) }
				if v.Duration > 0 { parts = append(parts, formatDuration(v.Duration)) }
				if v.Views > 0 { parts = append(parts, fmt.Sprintf("%d views", v.Views)) }
				u := v.URL; if u == "" { u = "https://sepiasearch.org/videos/watch/" + v.UUID }
				var ts int64; if v.CreatedAt != "" { if t, err := time.Parse(time.RFC3339, v.CreatedAt); err == nil { ts = t.UnixMilli() } }
...
func newGoogleVideos(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "google-videos",
		BuildURL: func(q string, opts SearchOptions) string { return "https://www.google.com/search?tbm=vid&q=" + url.QueryEscape(q) },
		Headers: map[string]string{"Cookie": "CONSENT=YES+"},
		Parse: func(body string, max int) ([]SearchResult, error) {
			results, _ := parseGoogleResults(body, max)
			for i := range results { results[i].Engine = "google-videos"; results[i].Category = "video" }
			return results, nil
		},
	})(config)
}
...
func newBingVideos(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "bing-videos",
		BuildURL: func(q string, opts SearchOptions) string { return "https://www.bing.com/videos/search?q=" + url.QueryEscape(q) },
		Parse: func(body string, max int) ([]SearchResult, error) {
			results, _ := parseBingResults(body, max)
			for i := range results { results[i].Engine = "bing-videos"; results[i].Category = "video" }
			return results, nil
		},
	})(config)
}
...
func (e *pipedEngine) Search(query string, opts SearchOptions, headers map[string]string) ([]SearchResult, error) {
	...
	var all []SearchResult; seen := map[string]bool{}
	for _, inst := range instances {
		if len(all) >= opts.NumResults { break }
		client := NewHTTPClient(time.Duration(e.config.Timeout) * time.Millisecond)
		...
		var data struct{ Items []struct{ URL, Title, UploaderName, UploadedDate string; Views, Duration int } `json:"items"` }
		if err := json.Unmarshal([]byte(resp), &data); err != nil { continue }
		for _, item := range data.Items {
			if len(all) >= opts.NumResults || item.Title == "" || item.URL == "" || seen[item.URL] { continue }
			seen[item.URL] = true
			var parts []string
			if item.UploaderName != "" { parts = append(parts, item.UploaderName) }
			if item.Views > 0 { parts = append(parts, formatViews(int64(item.Views))+" views") }
			if item.Duration > 0 { parts = append(parts, formatDuration(item.Duration)) }
			var ts int64; if item.UploadedDate != "" { if t, err := time.Parse(time.RFC3339, item.UploadedDate); err == nil { ts = t.UnixMilli() } }
...
func (e *invidiousEngine) Search(query string, opts SearchOptions, headers map[string]string) ([]SearchResult, error) {
	...
	for _, inst := range instances {
		...
		if err != nil || status < 200 || status >= 400 { continue }
		var items []struct{ VideoID, Title, Author, PublishedText, Description string; ViewCount int64; LengthSeconds int }
		if err := json.Unmarshal([]byte(resp), &items); err != nil { continue }
		for _, item := range items {
			if len(all) >= opts.NumResults || item.Title == "" || item.VideoID == "" || seen[item.VideoID] { continue }
			seen[item.VideoID] = true
			var parts []string
			if item.Author != "" { parts = append(parts, item.Author) }
			if item.ViewCount > 0 { parts = append(parts, formatViews(item.ViewCount)+" views") }
			if item.LengthSeconds > 0 { parts = append(parts, formatDuration(item.LengthSeconds)) }
			if item.PublishedText != "" { parts = append(parts, item.PublishedText) }
			snippet := strings.Join(parts, " · "); if snippet == "" { snippet = truncate(item.Description, 300) }
...
func newSogouVideos(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct{ Data *struct{ List []struct{ TitleEsc, URL, Site, Date, Duration string } `json:"list"` } `json:"data"` }
			if err := json.Unmarshal(data, &resp); err != nil || resp.Data == nil { return nil, nil }
			var results []SearchResult
			for i, item := range resp.Data.List {
				if i >= max || item.TitleEsc == "" || item.URL == "" { break }
				u := item.URL; if strings.HasPrefix(u, "/") { u = "https://v.sogou.com" + u }
				parts := []string{}
				if item.Site != "" { parts = append(parts, item.Site) }
				if item.Duration != "" { parts = append(parts, item.Duration) }
				var ts int64; if item.Date != "" { if t, err := time.Parse("2006-01-02", item.Date); err == nil { ts = t.UnixMilli() } }
...
func formatViews(views int64) string {
	if views >= 1_000_000 { return fmt.Sprintf("%.1fM", float64(views)/1_000_000) }
	if views >= 1_000 { return fmt.Sprintf("%.1fK", float64(views)/1_000) }
