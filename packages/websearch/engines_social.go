...
func newDouban(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "douban",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://www.douban.com/search?q=" + url.QueryEscape(q)
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: "", Engine: "douban", Position: pos, Category: "social"})
			}
			return results, nil
		},
	})(config)
...
func newMastodon(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Accounts []struct {
					URI, Username, DisplayName, Note, CreatedAt string
...
func newLemmy(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://lemmy.ml/api/v3/search?q=" + url.QueryEscape(q) + "&type_=Posts&limit=" + strconv.Itoa(minInt(n, 50))
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Posts []struct {
			...
			for i, p := range resp.Posts {
				...
				up, down, cmt := 0, 0, 0
				if p.Counts != nil {
					up, down, cmt = p.Counts.Upvotes, p.Counts.Downvotes, p.Counts.Comments
				}
				parts := []string{}
				if author != "" {
				...
				if cmt > 0 {
					parts = append(parts, fmt.Sprintf("%d comments", cmt))
				}
				var ts int64
				if p.Post.Published != "" {
					if t, err := time.Parse(time.RFC3339, p.Post.Published); err == nil {
						ts = t.UnixMilli()
					}
				}
...
func newDiscourse(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://meta.discourse.org/search.json?q=" + url.QueryEscape(q) + "+order:latest"
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Topics []struct {
			...
			for i, p := range resp.Posts {
				...
				} else if comments > 1 && t.Closed {
					parts = append(parts, "closed")
				}
				var ts int64
				if t.CreatedAt != "" {
					if tp, err := time.Parse(time.RFC3339, t.CreatedAt); err == nil {
						ts = tp.UnixMilli()
					}
				}
...
func newBoardreader(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "boardreader",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://boardreader.com/search.php?q=" + url.QueryEscape(q)
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				u := m[1]
				if !strings.HasPrefix(u, "http") {
					u = "https://boardreader.com" + u
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: u, Snippet: StripHTML(m[3]), Engine: "boardreader", Position: pos, Category: "social"})
			}
			return results, nil
		},
	})(config)
...
func newTootfinder(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://tootfinder.app/api/v1/search?q=" + url.QueryEscape(q) + "&count=" + strconv.Itoa(minInt(n, 20))
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Results []struct{ URI, Content, AccountDisplayName, CreatedAt string } `json:"results"`
			}
			if err := json.Unmarshal(data, &resp); err != nil {
				return nil, nil
			}
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if !strings.HasPrefix(href, "http") {
					u = "https://bandcamp.com" + href
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: u, Snippet: StripHTML(m[3]), Engine: "bandcamp", Position: pos, Category: "music"})
			}
			return results, nil
		},
	})(config)
...
func newDeezer(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://api.deezer.com/search?q=" + url.QueryEscape(q) + "&limit=" + strconv.Itoa(minInt(n, 25))
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Data []struct {
					...
					Album             *struct{ Title string } `json:"album"`
				} `json:"data"`
			}
			if err := json.Unmarshal(data, &resp); err != nil {
				return nil, nil
			...
			for i, t := range resp.Data {
				if i >= max || t.Type != "track" || t.Title == "" || t.Link == "" {
					break
				}
...
func newMixcloud(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://api.mixcloud.com/search/?q=" + url.QueryEscape(q) + "&type=cloudcast&limit=" + strconv.Itoa(minInt(n, 20))
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Data []struct {
			...
			var resp struct {
				Tracks *struct {
					Items []struct {
						...
						Album          *struct{ Name, HREF string } `json:"album"`
					} `json:"items"`
				} `json:"tracks"`
			}
			if err := json.Unmarshal(data, &resp); err != nil || resp.Tracks == nil {
				return nil, nil
			}
			var results []SearchResult
			for i, t := range resp.Tracks.Items {
				if i >= max || t.Name == "" || t.HREF == "" {
					break
				}
				artists := []string{}
...
func newYandexMusic(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://api.music.yandex.net/search?text=" + url.QueryEscape(q) + "&page=0&type=track"
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Result *struct {
					Tracks *struct {
						Results []struct {
							ID, Title string
							Artists   []struct{ Name string }  `json:"artists"`
							Albums    []struct{ Title string } `json:"albums"`
						} `json:"results"`
					} `json:"tracks"`
				} `json:"result"`
			}
			if err := json.Unmarshal(data, &resp); err != nil || resp.Result == nil || resp.Result.Tracks == nil {
				return nil, nil
			}
			var results []SearchResult
...
func newFyyd(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://api.fyyd.de/0.2/search/podcast?term=" + url.QueryEscape(q) + "&count=" + strconv.Itoa(minInt(n, 20))
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Data []struct {
	...
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			...
			for i, h := range resp.Hits {
				...
				tag := ""
				parts := strings.Split(h.Tags, ",")
				if len(parts) > 0 {
					tag = strings.TrimSpace(parts[0])
				}
				if tag == "" {
					tag = fmt.Sprintf("Pixabay %d", h.ID)
				}
				cat := "image"
				if h.Type == "video" {
					cat = "video"
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Photos []struct {
					...
					URL, Alt     string
					Photographer string
					Src          struct{ Medium string } `json:"src"`
				} `json:"photos"`
			}
			if err := json.Unmarshal(data, &resp); err != nil {
				return nil, nil
...
func newFlickr(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			...
			if err := json.Unmarshal(data, &resp); err != nil || resp.Photos == nil {
				return nil, nil
			}
			var results []SearchResult
...
func newDeviantArt(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "deviantart",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://www.deviantart.com/search?q=" + url.QueryEscape(q)
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if !strings.HasPrefix(href, "http") {
					href = "https://www.deviantart.com" + href
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: href, Snippet: "", Engine: "deviantart", Position: pos, Category: "image"})
			}
			return results, nil
		},
	})(config)
...
func newImgur(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://api.imgur.com/3/gallery/search/viral/all/" + strconv.Itoa(minInt(n, 20)) + "?q=" + url.QueryEscape(q)
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Data []struct {
...
func newPixiv(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "pixiv",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://www.pixiv.net/en/search?word=" + url.QueryEscape(q) + "&mode=all&order=date"
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				pos++
				results = append(results, SearchResult{Title: title, URL: "https://www.pixiv.net/en/artworks/" + m[1], Snippet: "", Engine: "pixiv", Position: pos, Category: "image"})
			}
			return results, nil
		},
	})(config)
...
func new500px(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "500px",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://500px.com/search?q=" + url.QueryEscape(q) + "&type=photos"
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				pos++
				results = append(results, SearchResult{Title: title, URL: "https://500px.com" + m[1], Snippet: "", Engine: "500px", Position: pos, Category: "image"})
			}
			return results, nil
		},
	})(config)
...
func newWallhaven(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://wallhaven.cc/api/v1/search?q=" + url.QueryEscape(q) + "&page=1"
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
...
func newOpenverse(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://api.openverse.engineering/v1/images/?q=" + url.QueryEscape(q) + "&page_size=" + strconv.Itoa(minInt(n, 20))
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Results []struct{ ID, Title, URL, Creator, Thumbnail string } `json:"results"`
			}
			if err := json.Unmarshal(data, &resp); err != nil {
				return nil, nil
			}
...
func newAdobeStock(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "adobe-stock",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://stock.adobe.com/de/Ajax/Search?k=" + url.QueryEscape(q) + "&limit=" + strconv.Itoa(minInt(20, 20)) + "&order=relevance&search_page=1&search_type=pagination"
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var raw struct {
			...
			for _, item := range raw.Items {
				if len(results) >= max || item.ContentURL == "" || item.Title == "" {
					break
				}
				pos++
				results = append(results, SearchResult{Title: item.Title, URL: item.ContentURL, Snippet: fmt.Sprintf("%s · %s", item.AssetType, item.Author), Engine: "adobe-stock", Position: pos, Category: "image"})
			}
			return results, nil
		},
	})(config)
...
func newGoogleImages(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "google-images",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://www.google.com/search?tbm=isch&q=" + url.QueryEscape(q)
		},
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				title := m[2]
				if title == "" || m[1] == "" || strings.HasPrefix(m[1], "data:") {
					continue
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: m[1], Snippet: "", Engine: "google-images", Position: pos, Category: "image"})
			}
			return results, nil
		},
	})(config)
...
func newNaver(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "naver",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://search.naver.com/search.naver?where=web&query=" + url.QueryEscape(q) + "&start=1"
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				pos++
				results = append(results, SearchResult{Title: title, URL: m[1], Snippet: "", Engine: "naver", Position: pos})
			}
			return results, nil
		},
	})(config)
...
func newStartpage(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "startpage",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://www.startpage.com/do/search?q=" + url.QueryEscape(q) + "&cat=web&language=english"
		},
		Headers: map[string]string{
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: StripHTML(m[3]), Engine: "startpage", Position: pos})
			}
			// 对应 TS fallbackRegex
			if len(results) == 0 {
				...
				for _, m := range fbRe.FindAllStringSubmatch(body, -1) {
					...
					title := StripHTML(m[2])
					url := m[1]
					if title == "" || url == "" || strings.Contains(url, "startpage.com") {
						continue
					}
					pos++
					results = append(results, SearchResult{Title: title, URL: url, Snippet: "", Engine: "startpage", Position: pos})
				}
			}
			return results, nil
		},
...
func newQwant(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			...
			if err := json.Unmarshal(data, &resp); err != nil || resp.Data == nil || resp.Data.Result == nil || resp.Data.Result.Items == nil {
				return nil, nil
			}
			var results []SearchResult
			for _, section := range resp.Data.Result.Items.Mainline {
				...
				for _, item := range section.Items {
					if len(results) >= max || item.Title == "" || item.URL == "" {
						break
					}
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if !strings.HasPrefix(url, "http") {
					url = "https://yep.com" + url
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: StripHTML(m[3]), Engine: "yep", Position: pos})
			}
			return results, nil
		},
	})(config)
...
func newMojeek(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "mojeek",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://www.mojeek.com/search?q=" + url.QueryEscape(q) + "&s=6"
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if strings.HasPrefix(url, "/") {
					url = "https://www.mojeek.com" + url
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: StripHTML(m[3]), Engine: "mojeek", Position: pos})
			}
			return results, nil
		},
	})(config)
	...
	return newHTMLScraperEngine(htmlScraperConfig{
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if !strings.HasPrefix(url, "http") {
					url = "https://search.seznam.cz" + url
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: StripHTML(m[3]), Engine: "seznam", Position: pos})
			}
			// 备用模式：宽松匹配 h2/h3
			if len(results) == 0 {
				fbRe := regexp.MustCompile(`<a[^>]*href="(https?:\/\/[^"]+)"[^>]*>[\s\S]*?<h[23][^>]*>([\s\S]*?)<\/h[23]>`)
				for _, m := range fbRe.FindAllStringSubmatch(body, -1) {
					...
					title := StripHTML(m[2])
					url := m[1]
					if title == "" || url == "" || strings.Contains(url, "seznam.cz") {
						continue
					}
					pos++
					results = append(results, SearchResult{Title: title, URL: url, Snippet: "", Engine: "seznam", Position: pos})
				}
			}
			return results, nil
		},
...
func newAol(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "aol",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://search.aol.com/aol/search?q=" + url.QueryEscape(q)
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if !strings.HasPrefix(url, "http") {
					url = "https://search.aol.com" + url
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: StripHTML(m[3]), Engine: "aol", Position: pos})
			}
			return results, nil
		},
	})(config)
...
func newGmx(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "gmx",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://suche.gmx.net/search?q=" + url.QueryEscape(q)
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if !strings.HasPrefix(url, "http") {
					url = "https://suche.gmx.net" + url
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: StripHTML(m[3]), Engine: "gmx", Position: pos})
			}
			return results, nil
		},
	})(config)
...
func newMwmbl(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Results []struct{ Title, URL, Snippet string } `json:"results"`
			}
			if err := json.Unmarshal(data, &resp); err != nil {
				return nil, nil
			}
...
func newGrokipedia(config EngineConfig) SearchEngine {
	return newJSONAPIEngine(jsonAPIConfig{
		...
		URL: func(q string, n int) string {
			return "https://grokipedia.com/api/full-text-search?query=" + url.QueryEscape(q) + "&limit=" + strconv.Itoa(minInt(n, 20)) + "&offset=0"
		},
		Parse: func(data []byte, max int) ([]SearchResult, error) {
			var resp struct {
				Results []struct{ Slug, Title, Snippet string } `json:"results"`
			}
			if err := json.Unmarshal(data, &resp); err != nil {
				return nil, nil
			}
	...
	return newHTMLScraperEngine(htmlScraperConfig{
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: "", Engine: "360search", Position: pos})
			}
			return results, nil
		},
	})(config)
...
func newChinaSo(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "chinaso",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://www.chinaso.com/search/?q=" + url.QueryEscape(q)
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if !strings.HasPrefix(url, "http") {
					url = "https://www.chinaso.com" + url
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: StripHTML(m[3]), Engine: "chinaso", Position: pos})
			}
			return results, nil
		},
	})(config)
...
func newQuark(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "quark",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://quark.sm.cn/s?q=" + url.QueryEscape(q) + "&from=smor&safe=1"
		},
		Parse: func(body string, max int) ([]SearchResult, error) {
			var results []SearchResult
			pos := 0
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				if !strings.HasPrefix(url, "http") {
					url = "https://quark.sm.cn" + url
				}
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: StripHTML(m[3]), Engine: "quark", Position: pos})
			}
			return results, nil
		},
	})(config)
...
func newYandex(config EngineConfig) SearchEngine {
	return newHTMLScraperEngine(htmlScraperConfig{
		Name: "yandex",
		BuildURL: func(q string, opts SearchOptions) string {
			return "https://yandex.com/search/site/?text=" + url.QueryEscape(q) + "&tmpl_version=releases&web=1&frame=1&searchid=3131712"
		},
		Headers: map[string]string{
			"User-Agent":      "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36",
			"Accept-Language": "en-US,en;q=0.9",
		...
		Parse: func(body string, max int) ([]SearchResult, error) {
			...
			for _, m := range re.FindAllStringSubmatch(body, -1) {
				...
				pos++
				results = append(results, SearchResult{Title: title, URL: url, Snippet: snippet, Engine: "yandex", Position: pos})
			}
			return results, nil
		},
	})(config)
