...
func (db *Database) MigrateToDisk(name string) (VectorCollection, error) {
	...
	if !IsSSD {
		log.Printf("vectordb: migration skipped for %q (not on SSD)", name)
		return hnswCol, nil
	}

	log.Printf("vectordb: migrating %q (%d items) to disk...", name, hnswCol.ItemCount())
	...
	log.Printf("vectordb: migrated %q to disk (%d items)", name, vc.ItemCount())
	return vc, nil
}
...
func (db *Database) MaybeMigrate(name string) {
	...
	go func() {
		if _, err := db.MigrateToDisk(name); err != nil {
			log.Printf("vectordb: auto-migration for %q failed: %v", name, err)
		}
	}()
}
