...
//
// This file implements Unix (Linux/macOS) platform mmap memory-mapped reader.
// Uses syscall.Mmap for zero-copy reads, suitable for large index files.
package storage
...
// mmapReader implements mmap-based disk index reader.
//
// Uses memory mapping for zero-copy reads, suitable for Linux and macOS.
// Mapped region is loaded on-demand by OS, supports index files larger than physical memory.
type mmapReader struct {
	file      *os.File      // underlying file handle
	data      []byte        // mmap mapped byte slice
	meta      GraphMetadata // parsed graph metadata
	blockSize uint64        // data block size
}

// platformOpenReader creates Unix platform mmap reader.
//
// Parameters:
...
func platformOpenReader(path string, readOnly bool) (DiskIndexReader, error) {
	// Determine open mode and mmap protection flags
	flags := os.O_RDONLY
	...
	if err != nil {
		f.Close()
		return nil, fmt.Errorf("create mmap failed: %w", err)
	}
...
func (r *mmapReader) calcOffset(nodeID uint64) uint64 {
...
func (r *mmapReader) ReadNode(nodeID uint64, buf []byte) error {
...
func (r *mmapReader) ReadNeighbors(nodeID uint64) ([]uint32, error) {
	// Validate nodeID range
	if nodeID >= r.meta.NumPoints {
...
func (r *mmapReader) ReadVector(nodeID uint64, vec []float32) error {
	// Validate nodeID range
	if nodeID >= r.meta.NumPoints {
		return ErrNodeNotFound
	}
...
func (r *mmapReader) Metadata() *GraphMetadata {
	return &r.meta
}

// Warmup preloads specified nodes into memory.
//
// For mmap implementation, this triggers OS page prefetch.
// Accesses first byte of node data to trigger page load.
//
// Parameters:
//   - nodeIDs: list of node IDs to warmup
func (r *mmapReader) Warmup(nodeIDs []uint64) error {
