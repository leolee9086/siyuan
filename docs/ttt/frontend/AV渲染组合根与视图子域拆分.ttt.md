...
- **2026-07-27**：Properties 字段管理 HTML 从 Panel 聚合入口归入 `col/properties` 子域，删除 Panel 包装、依赖参数和只服务该包装的碎片类型；三个消费者直达唯一实现。子域网关直接登记 Emoji、转义、i18n 与列图标真实所有者，不经其它 imports 网关。专项 `1/1`、Node `190/190`、Protyle 契约类型、新子域 lint 与网关门禁通过；生产图 `2203 / 424 / SCC 668`，Properties 子域位于 SCC 外，目标 Panel/Drag 与 ColOps/Panel 环归零。
