...
- **2026-07-28**：Phase 5 继续核验导出返回链。实际对比确认 `layout/util.environment` 的配置/存储访问器为安全可空读取，严格通用访问器并不等价；导出域改为直达已有 `getSafeSiyuanConfig/getSafeSiyuanStorage`，根网关两项零消费者出口同时删除。存储与菜单回归 `6/6` 固定缺失 storage 默认值、历史值规范化和原对象写回；目标类型诊断 `0`、根导出网关 lint、imports 多跳 `0` 与 diff 检查通过。代表环 `159 -> 158`，首环不再经过图片导出与 Wnd，推进到 `protyle/export -> util/assets -> noRelyPCFunction`。图片目录仍有既有 `11 > 10` 结构门禁及三个消费者文件存量 lint，未记为全过。
- **2026-07-28**：继续沿导出首环核对 `assets -> noRelyPCFunction`，确认 assets 只消费 `getWorkspaceName`。项目已有 `processTitle` 唯一完整实现，额外处理浏览器不返回绝对路径及语言尚未加载；综合平台版直接假定路径存在，且仅有 assets/topBar 两个消费者。两处改为直达完整实现并删除较弱重复函数及专用路径导入。工作空间专项 `2/2`、Node `204/204`、新增测试目标类型诊断 `0`、imports 多跳 `0` 与 diff 检查通过；代表环 `158 -> 142`，CommonMenuItem/导出链完全退出首环，当前首环推进到 `navigation.initMultiMenu -> menus/util -> editor/utils.openFileById`。
- **2026-07-28**：导航打开菜单的六处具体 `openFileById` 改用既有完整 `AppFacade.openBlock`；实际参数均已由 `AppBlockNavigation` 覆盖，桌面组合根继续委托同一实现，移动端不进入该菜单分支。专项 `2/2` 固定折叠文档右侧打开和笔记本新页签载荷，Node `204/204`、imports 多跳 `0` 与 diff 检查通过；代表环保持 `142`，但 `navigation.initMultiMenu -> menus/util -> openFileById` 首环归零，新首环转为 `navigation -> search/spread`。`menus/util` 仍有既有精确可选、数组首项与全局配置严格诊断，未误记为本批新增。
- **2026-07-28**：沿新首环完成 Search 配置与结果预览职责拆分。桌面 `genQueryHTML/updateConfig` 从综合 `search/util.ts` 迁入 `search/config`，调用方传入既有 `inputEvent`，配置子域不反向加载搜索执行链；`getArticle/getAttr` 分别迁入文章预览与结果元数据领域，全部消费者直达真实声明。文章当前选择进入统一 SForge Symbol 注册表，Resize/Intersection Observer 构造提升为共享唯一工厂并同步迁移 Panel，未重复实现。专项 Search/文章/Panel 回归 `12/12`、Node `204/204`、新模块 lint、目标类型诊断 `0`、imports 多跳 `0` 与 diff 检查通过；代表环 `142 -> 134`，`search/util -> ... -> navigation -> search/spread -> inputEvent -> onSearch -> search/util` 闭环退出，CommonMenuItem 与导出链继续保持在首环外。
