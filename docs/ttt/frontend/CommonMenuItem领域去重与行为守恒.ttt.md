...
>
> **当前目标**：重命名与移动动作已唯一化；逐格式固定导出菜单的当前行为证据。
>
> **下一步任务**：完整比较根 `exportMd` 与 `export/` 下每个拆分动作的菜单结构、请求、消息、平台桥接和错误传播，再建立桌面/移动/浏览器行为测试。
...
- [ ] 按当前生效根实现修正每一种格式的拆分动作，不沿用已确认漂移的旧行为。
- [ ] 建立桌面、移动、浏览器菜单顺序及关键动作测试。
- [ ] 所有调用方直达导出菜单唯一所有者，删除根文件对应实现。

### Phase 4：剩余能力与旧入口删除

- [ ] 逐项核对并迁移 `openAttr/copySubMenu/openMenu/openWechatNotify/openFileWechatNotify/openFileAttr`。
