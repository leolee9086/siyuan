...
>
> **当前目标**：分离调用方必需响应处理器的上传传输路径与默认编辑器/AV 后处理，解除 `Asset -> upload -> cell.update -> select/relation/view -> transaction` 返回路径。
>
> **下一步任务**：建立 callback、默认后处理、校验失败、取消确认和 XHR 状态行为测试，提取共享校验/传输核心及必需响应处理器入口。
