...
- **2026-07-27**：Cronjob 不再依赖具体 Tab、综合 `tabUtil` 或编辑器导航实现，统一使用完整 `LayoutTab`、直达 `layout/query/dockByType` 与完整 AppFacade；单一 `CronjobDomain` 覆盖 class 全部公开表面，并以 Symbol 厂牌和 `PublicInstanceLooksLike` 双向契约校验。Model 初始化按既有生命周期协议恢复为先构造状态、再连接 WebSocket，不改变更新和销毁行为。Cronjob 返回边退出首环，生产图为 `2316` 个节点、`154` 条代表环；目标类型诊断 `0`、Node `200/200`、Protyle 契约和 imports 多跳门禁通过。
- **2026-07-27**：桌面 Files 与移动 MobileFiles 的“重建索引”动作改为直达唯一 `util/file/rebuildDataIndex` 命令，不再经同时聚合退出、同步、进度和内核错误 UI 的 `dialog/processSystem.ts`。两个历史 processSystem 入口中完全重复的实现已删除，仅将旧公开名称重导出到同一命令；缓存失效、持久化、请求和完成回调顺序不变。Files 的 processSystem 返回链归零，生产图 `2318 / 150`；新增领域 lint、目标类型诊断、Node `200/200` 和网关多跳门禁通过。
- **2026-07-27**：Files 更多菜单与 MobileFiles 不再为完整排序子菜单加载 `menus/navigation.ts` 上帝模块。唯一 `navigation/sortMenu` 完整保留 15 种模式、固定顺序、五个分隔符、选中态和宿主回调，三个调用点直达真实所有者，旧模块无兼容出口。排序专项 `2/2`、Node `202/202`、新增领域 lint、目标类型诊断与 imports 多跳门禁通过；navigation 返回边退出 Files 首环，生产图 `2320 / 172`，首环推进到笔记本挂载能力。
- **2026-07-27**：Files 更多菜单、移动文件树、移动菜单、空工作区和中心页签的新建笔记本入口不再加载综合 `util/file/mount.ts`。普通与加密创建分别进入完整 factory 领域并共享既有完整 `IDialog` 抽象，名称、导入、口令、原子创建、键盘和防重入语义保持；所有调用方直达真实所有者，mount 中旧实现及专属 Electron/Obsidian/名称规则依赖删除。生产图 `2324 / 149`，moreMenu 与 mount 返回边退出首环；新增领域 lint、目标类型诊断、Node `202/202` 和网关多跳门禁通过。
- **2026-07-27**：按 [Files 文件树领域目录拆分](./Files文件树领域目录拆分.ttt.md) 建立 `Files/focus` 子域。当前编辑器定位命令改依赖完整 Files、LayoutTab、Editor 领域及 Symbol 守卫，具体 Files/Tab/Editor class 从该路径归零；全局命令和 actions 直达新所有者，actions 的 Dock 查询直达 Query。删除重复结构式 Files 守卫后唯一身份判别继续由双向契约覆盖。目标路径退出首环，生产图 `2326 / 171`；focus lint、目标类型诊断、Node `202/202` 和 imports 多跳门禁通过。
- **2026-07-27**：Files closeElement 与 MobileFiles 的加密笔记本打开动作直达独立 notebook access factory，综合 mount 返回边归零；具体 Dialog 仅在 factory 边界，行为依赖完整 `IDialog`。生产图 `2328 / 149`，closeElement 退出首环；新增领域 lint、目标类型诊断、Node `202/202` 和 imports 多跳门禁通过。
