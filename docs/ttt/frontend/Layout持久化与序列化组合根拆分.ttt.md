...
- **2026-07-28**：反序列化调用方移除 `layout/util.ts` 转发，`util.environment` 与 `util.guard` 均改依赖布局完整领域类型；`Layout/Wnd/Tab` 具体实现只留在需要构造或运行时装配的边界。该批使 Wnd/Layout 反序列化环退出生产图，代表环由 `55` 继续降至 `34`；反序列化字段与启动时序未改，后续继续处理 `tabUtil -> util/file/newFile`。
