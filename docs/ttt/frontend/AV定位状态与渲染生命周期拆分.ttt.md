...
- [x] 解除完成呈现经 `scrollCenter -> layout/Wnd -> editor` 返回 AV 根渲染的既有布局回路。

## 下一步任务

1. 补强 Presentation 的表格首行与看板横向校正端到端行为测试。
2. 保持纯目标滚动原语与 Selection/Range 编辑器编排分层，后续调用方按真实语义逐步直达。
3. 跟随 AV 渲染专项处理新的最短环 `render -> gallery -> render.table -> calc -> transaction.promise`。
...
- **2026-07-27**：将明确 DOM 目标的 `start/center/nearest` 计算归入唯一 `scrollTargetIntoView` 原语，输入只包含滚动容器、目标和严格判别参数；`start` 间距由编辑器编排显式提供，Selection/Range、块查找和配置读取继续留在完整 `scrollCenter`，没有复制算法或注入 callback Port。Presentation 经本阶段 `imports.ts` 直达真实滚动实现，不再加载 `highlightById -> helpers -> layout/Wnd`。滚动与定位专项 `7/7`、Node `193/193`、Protyle 契约类型、目标 lint、网关多跳和 diff 检查通过；生产图 `2221 / 391 / SCC 648`，仍为唯一循环 SCC，Presentation `finish.ts` 已完全退出 SCC。表格首行和看板横向端到端测试继续列为补强项，不据此提前归档整个 AV 专项。
