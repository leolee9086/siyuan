...
- npm `siyuan` 官方 `latest` dist-tag 的公开类型是插件生态和共享数据格式的外部兼容基线，不是临时脚手架或可选依赖；本地领域类型只能在保持可赋值兼容的前提下强化已初始化不变量。`check:siyuan-types` 必须在依赖更新后确认安装包、锁文件和官方 latest 三者一致。
...
- 上游类型基线使用 npm 包 `siyuan` 的官方 `latest` dist-tag，公开声明提供 `Plugin`、`EventBus` 及插件生态共享的数据结构；生产构建仅作 type-only 引用，但其契约约束是长期、强制且不可由本地玩具类型替代的。
...
- **2026-07-28**：完成 Dialog 退出路径回归修复。提交 `24cb30973` 合并时把桌面默认 `closeButtonPosition: "outside"` 的关闭按钮生成结果改为空字符串，导致设置与 Agent Tab 非模态浮窗同时缺少显式关闭入口；基础生成器现恢复桌面/移动统一生成，并依据关闭按钮实际存在性安排全屏按钮和标题栏右侧空间，避免两图标重叠。设置面板继续保持非模态语义：透明、穿透且不响应遮罩关闭，调用处已明确注释退出仅由显式关闭按钮和全局 `Esc` 提供。新增 `dialogCloseButton.runtime.test.ts` 与 `dialogEscape.runtime.test.ts`，覆盖按钮生成/隐藏、全屏错位、事件绑定、Calibur 根路由和顶层 Dialog 销毁，共 `8/8` 通过；真实桌面页确认设置关闭按钮可见、遮罩不拦截主界面、搜索框聚焦时 `Esc` 可关闭。Agent 浮窗的实际副本创建仍需继续验证，因此 Phase 4 的跨宿主浮窗验收保持未完成。
