...
- [ ] **Phase 1: 核心引擎基础 — 配置 + LLM 客户端 + 上下文构建器**
  ...
  - **验收标准**:
    ...
    - 配置可从 JSON 加载
    - LLM 客户端可完成单轮对话
    - 上下文构建器可组装 System + Memories + History
...
- [ ] **Phase 2: Agent 主循环 + 工具注册表**
  ...
  - **范围**:
    ...
    2. Agent 主循环 (#5): Go for 循环 + `sync.WaitGroup` 并行工具执行，**高复杂度**，~450 行
  - **验收标准**:
    - ReAct 循环可迭代执行（Observe → Decide → Act）
    - 工具可注册和调用
...
- [ ] **Phase 3: 安全层**
  - **背景**: 多层级安全沙箱，包含工作项 #7-#12（设计文档 §3.2），~990 行
  - **范围**: PromptGuard、SessionBudget、FileGuard、ShellSandbox、AuditLog、SecurityDoctor
