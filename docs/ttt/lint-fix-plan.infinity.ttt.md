...
最后更新: 2026-02-26T13:36Z
状态: 批次0完成，进入批次5（按文件错误数从多到少修复）
...
- [x] 将 `*.backup.ts`、`*.old.ts` 加入 eslint ignores（消除 ~955 无效错误）
- [x] 重新运行 lint 确认基线
...
### 批次 5: 拆分后的 lint 错误修复（按文件错误数从多到少）

修复策略：按文件错误数从多到少逐个修复，优先处理错误密度最高的文件。
详见：[lint-error-by-file.md](./lint-error-by-file.md)
...
- [x] 批次 0: 配置清理（排除备份文件）✅ 完成（2026-02-26）
...
- [ ] 批次 5: lint 错误修复（下一步）→ [lint-error-by-file.md](./lint-error-by-file.md) 查看修复优先级
