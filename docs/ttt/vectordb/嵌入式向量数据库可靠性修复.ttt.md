...
- [x] **Phase 0-2: DiskVamana 小集合搜索错误根因调查与修复（P0）**【已完成 2026-07-11】
  ...
  - **完成状态**：小集合搜索修复及其回归用例已完成；解除 Phase 0-3/0-4 阻塞后，`go test ./... -short -count=20 -timeout 600s` 全包门禁通过，根包 236.867 秒、Vamana 子包 191.376 秒，无最近邻断言失败。
...
- [x] **Phase 0-4: Windows mmap 生命周期与文件占用修复（P0）**【已完成 2026-07-11】
  ...
  - **验证结果**：`TestBug_VamanaInsertUpdateRace` 在 `-race -count=20` 下通过；Windows reader 关闭测试与集合目录立即重命名/删除测试通过；相关生命周期门禁在 `-race -count=5` 下通过。

- [-] **Phase 0-5: 错误传播修复与稳定门禁建立（P0）**【核心修复与自动化门禁已完成，持续 30 分钟压力运行待执行】
  ...
  - **失败记录位置**：本文档"⚠️ 失败记录"章节。
  - **错误传播修复**：公开集合契约新增错误感知的搜索和删除路径；DiskVamana 搜索、删除、刷新、关闭错误不再转换为空结果或成功；数据库打开已存在但损坏的磁盘集合时不再静默回退为“集合不存在”。
  - **稳定错误**：已定义关闭、维度、损坏、忙、只读和持久化失败错误，并通过 `errors.Is` 分类底层 Vamana/storage 错误；磁盘搜索维度错误返回稳定错误；关闭时删除位图保存失败会返回错误，同时仍继续关闭 reader 释放 mmap。
  - **自动化门禁**：新增 32 个查询协程、4 个写协程和 1 个 checkpoint 协程的 `TestConcurrentStress_UpsertSearchFlushClose`，验证关闭错误、ID 唯一性、计数和重开持久化；`-race -count=5` 通过。
  - **全包结果**：`go test ./... -short -count=20 -timeout 600s` 通过；`go test -race ./... -short -count=5 -timeout 600s` 通过，根包 169.263 秒、Vamana 子包 147.122 秒。
  - **测试分层**：HNSW SIFT 10K 与 5K—20K NodeCache 构建属于规模/性能测试，已与同目录其他规模测试一致，在 `-short` 下跳过；非 `-short` 定向运行仍保留，HNSW SIFT 10K 非 race 5.22 秒、race 122.40 秒，NodeCache 10K race 25.94 秒。
  - **剩余验收**：尚未实际持续运行 30 分钟压力门禁，因此本阶段暂不归档为完全完成。
