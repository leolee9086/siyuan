...
const isFileTreeMoveRecord = (value: unknown): value is IFileTreeMoveRecord => {
    if (!isRecord(value)) {
        return false;
    }
    return typeof value.fromNotebook === "string" &&
        typeof value.fromPath === "string" &&
        typeof value.toNotebook === "string" &&
        typeof value.toPath === "string";
};

const getMovedFileName = (path: string) => path.slice(path.lastIndexOf("/") + 1);

const getMovedDocumentPath = (record: IFileTreeMoveRecord) => {
    if (record.newPath) {
        return record.newPath;
    }
    const fileName = getMovedFileName(record.fromPath);
    return record.toPath === "/" ? `/${fileName}` : `${record.toPath}/${fileName}`;
};

const toFileTreeMove = (value: unknown): IFileTreeMove | undefined => {
    if (!isFileTreeMoveRecord(value)) {
        return undefined;
    }
    return {
        fromNotebook: value.fromNotebook,
        fromPath: value.fromPath,
        toNotebook: value.toNotebook,
        toPath: value.toPath,
        newPath: getMovedDocumentPath(value),
    };
};

/** 将单文档或批量移动 WebSocket 数据统一为完整移动记录。 */
export const normalizeFileTreeMoves = (data: unknown): IFileTreeMove[] => {
    if (!isRecord(data)) {
        return [];
    }
    if (Array.isArray(data.moves)) {
        return data.moves.map(toFileTreeMove).filter((move): move is IFileTreeMove => Boolean(move));
    }
    const move = toFileTreeMove(data);
    return move ? [move] : [];
};
...
export const collectExpandedDocIDs = (
    ...
    childListElement?.querySelectorAll(".b3-list-item__arrow--open").forEach((arrowElement) => {
        const childItemElement = arrowElement.closest<HTMLElement>("li[data-node-id]");
        const childId = childItemElement?.dataset.nodeId;
        if (childId) {
            expandedDocIDs.add(childId);
        }
    });
};
...
export const updateMovedSubtree = (
    ...
    childListElement?.querySelectorAll<HTMLElement>("li[data-path]").forEach((item) => {
        ...
        if (currentPath) {
            updateMovedItemPath(item, remapMovedPath(currentPath, fromPath, newPath));
        }
    });
};
