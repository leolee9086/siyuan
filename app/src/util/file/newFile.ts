...
export const newFile = async (app: AppFacade, name?: string) => {
    ...
    if (name === undefined) {
        ...
        runNewDoc({
            ...
            currentPath,
            hasFocusTarget,
            name: await replaceFileName(name.trim()),
            onCreated: () => hideElements(["dialog"]),
        });
    }
};

export const newFileInProtyle = async (protyle: IProtyle, onCreated: (id: string, title: string) => void) => {
    runNewDoc({
        app: protyle.app,
...
export const newFileBySelect = async (protyle: IProtyle, selectText: string, nodeElement: HTMLElement, pathDir: string, targetNotebookId: string) => {
    const newFileName = await replaceFileName(selectText.trim() ? selectText.trim() : protyle.lute.BlockDOM2Content(nodeElement.outerHTML).replace(/\n/g, "").trim());
 ...
 */
export const newFileByRefHint = async (
    protyle: IProtyle,
    name: string,
    onCreated?: (id: string, title: string) => void,
    presetId?: string,
) => {
    const requestName = await replaceFileName(name.trim());
