...
/** 执行关闭其它或全部标签命令。 */
const executeCloseGroupDesktopGlobalCommand = ({ command }: GlobalCommandContext) => {
    const tab = getActiveTab(false);
...
/** 执行关闭左侧或右侧标签命令。 */
const executeCloseSideDesktopGlobalCommand = ({ command }: GlobalCommandContext) => {
    const tab = getActiveTab(false);
 ...
 */
export const executeDesktopCloseGlobalCommand = (context: GlobalCommandContext) => {
