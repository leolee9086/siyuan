...
const executeDirectDockMobileGlobalCommand = ({ command }: GlobalCommandContext) => {
    openDock(command);
    return true;
};
...
const executeGlobalSearchMobileGlobalCommand = ({ app }: GlobalCommandContext) => {
    popSearch(app);
    return true;
};

/** 执行移动端最近文档命令，命中 recentDocs 时打开最近文档菜单。 */
const executeRecentDocsMobileGlobalCommand = ({ app }: GlobalCommandContext) => {
    getRecentDocs(app);
    return true;
};
 ...
 */
export const executeMobileGlobalCommand = (context: GlobalCommandContext) => {
