...
import type {LayoutDomain, LayoutRuntimeDomain, LayoutWindow} from "../layout.types";
/** 导出查询算法使用的完整布局领域根。 */
export type {LayoutDomain};
/** 导出活动窗口查询使用的完整窗口领域根。 */
export type {LayoutWindow};
/** 导出完整运行时布局容器。 */
export type {LayoutRuntimeDomain};

/** 用途：Dock 聚合根。使用范围：按模型类型查询 Dock；解耦评估：完整公共领域表面，不加载具体 Dock class。 */
import type {DockDomain} from "../dock/dock.types";
/** 导出 Dock 聚合根。 */
