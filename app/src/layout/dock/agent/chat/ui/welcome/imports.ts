...
/** 用途：保存欢迎示例轮次；使用范围：请求发出前；解耦评估：会话持久化保持唯一实现。 */
import {saveSession} from "../../session/persistence/AgentChat.save";
/** 导出会话保存命令。 */
export {saveSession};
/** 用途：恢复冲突后的会话；使用范围：保存失败处理；解耦评估：冲突恢复保持在会话持久化领域。 */
...
/** 用途：渲染用户消息；使用范围：欢迎示例轮次；解耦评估：复用用户消息唯一 DOM 投影入口。 */
import {appendUserMessage} from "../../message/user/AgentChat.userMessage";
/** 导出用户消息追加命令。 */
export {appendUserMessage};

/** 用途：重建消息导航；使用范围：欢迎示例轮次；解耦评估：导航标记由导航领域统一维护。 */
import {rebuildNavMarkers} from "../navigation/AgentChat.navigation";
/** 导出导航重建命令。 */
export {rebuildNavMarkers};

/** 用途：生成会话标题；使用范围：欢迎示例轮次；解耦评估：标题生成规则由响应收尾领域统一维护。 */
import {tryGenerateTitle} from "../../stream/response/AgentChat.finish.methods";
/** 导出标题生成命令。 */
export {tryGenerateTitle};
/** 用途：控制流式状态；使用范围：欢迎示例轮次；解耦评估：流式按钮和只读状态由统一状态函数投影。 */
import {setStreaming} from "../feedback/AgentChat.streamingState";
/** 导出流式状态命令。 */
export {setStreaming};
/** 用途：回滚失败的用户条目；使用范围：欢迎示例错误处理；解耦评估：条目和 DOM 回滚由响应错误领域统一维护。 */
