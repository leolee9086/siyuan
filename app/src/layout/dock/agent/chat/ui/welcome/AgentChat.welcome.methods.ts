...
/** 用途：保存欢迎示例建立的用户轮次；使用范围：流式请求发出前；解耦评估：复用会话持久化唯一入口。 */
import {saveSession} from "./imports";
/** 用途：恢复保存冲突后的会话；使用范围：欢迎示例错误路径；解耦评估：复用会话重载唯一入口。 */
...
/** 用途：追加欢迎示例用户消息；使用范围：轮次建立；解耦评估：复用用户消息唯一 DOM 投影入口。 */
import {appendUserMessage} from "./imports";
/** 用途：重建消息导航标记；使用范围：用户消息追加后；解耦评估：导航状态只由导航领域维护。 */
import {rebuildNavMarkers} from "./imports";
/** 用途：为新会话生成标题；使用范围：首条欢迎示例消息；解耦评估：标题规则集中在响应收尾领域。 */
import {tryGenerateTitle} from "./imports";
/** 用途：切换流式交互状态；使用范围：欢迎示例请求开始和保存失败；解耦评估：流式 UI 状态由共享状态函数维护。 */
import {setStreaming} from "./imports";
/** 用途：回滚保存失败的用户条目；使用范围：欢迎示例冲突处理；解耦评估：条目与 DOM 回滚集中在响应错误领域。 */
import {rollbackUserEntry} from "./imports";

/** 建立示例提问对应的用户轮次，并在保存冲突时恢复会话。 */
