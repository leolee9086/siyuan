...
export function mountTiptapComposer(host: HTMLElement, onSend: () => void, onChange?: ComposerChangeCallback): ComposerHandle {
        ...
        editorProps: {
            ...
            handleKeyDown: function (_view, event) {
                ...
                if (event.key === "ArrowUp" && !suggestionMenu) {
                    const navigating = history.isBrowsing();
                    const isEmpty = _view.state.doc.childCount === 1 &&
                        _view.state.doc.firstChild?.childCount === 0;
                    if ((navigating || isEmpty) && history.has()) {
                        event.preventDefault();
                        const target = navigating
                            ? history.navigateUp()
                            : history.beginBrowsing(editor.state.doc.textContent);
                        editor.commands.setContent(target);
                        return true;
                    }
                }
                if (event.key === "ArrowDown" && !suggestionMenu && history.isBrowsing()) {
                    event.preventDefault();
                    editor.commands.setContent(history.navigateDown());
                    return true;
                }
    ...
    return {
        ...
        getSendData: function () {
            ...
            return {text: textParts.join("").trim(), blockHTML: editor.getHTML(), references: refs};
        },
        ...
        restoreHistory: (items: string[]) => history.restore(items),
