 ...
 */
function onElementMousedown(event: MouseEvent, files: FilesDomain, app: AppFacade): void {
    // 检查是否为鼠标中键点击
    if (event.button !== 1) {
        return;
    }
 ...
 * @同步豁免: UI构建
 */
export function setupElementMousedownHandler(files: FilesDomain, app: AppFacade): void {
