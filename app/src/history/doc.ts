...
const repoHistoryEditors = new WeakMap<HTMLElement, ProtyleDomain>();
let isLoading = false;
...
const renderRepo = async (element: HTMLElement, currentPage: number, id: string) => {
    ...
    pageInfoElement.classList.remove("fn__none");
    renderRepoFileList({files: response.data.files, element: listElement, showPath: false, showCompare: true});
...
export const openDocHistory = (options: {
    ...
    const previewRepoFile = (element: Element) => {
        ...
        renderRepoFile({
            app: options.app,
            element,
            contentElement: repoPreviewElement,
            onEditor: (editor) => {
