...
export const viewCards = (app: App, deckID: string, title: string, deckType: "Tree" | "" | "Notebook", cb?: (response: IWebSocketData) => void) => {
    ...
    <div class="block__icons">
        ...
        <span data-type="resetAll" class="block__icon block__icon--show b3-tooltips b3-tooltips__w${(deckType === "" && deckID === "") ? " fn__none" : ""}" aria-label="${siyuanI18n.reset}"><svg><use xlink:href='#iconUndo'></use></svg></span>
        <span class="fn__space"></span>
        <span data-type="previous" class="block__icon block__icon--show b3-tooltips b3-tooltips__w" disabled="disabled" aria-label="${siyuanI18n.previousLabel}"><svg><use xlink:href='#iconLeft'></use></svg></span>
        <span class="fn__space"></span>
        <span data-type="next" class="block__icon block__icon--show b3-tooltips b3-tooltips__w" disabled="disabled" aria-label="${siyuanI18n.nextLabel}"><svg><use xlink:href='#iconRight'></use></svg></span>
        <span class="fn__space"></span>
...
<div data-type="close" class="block__icon block__icon--show">
    ...
    <div class="${isMobile() ? "fn__flex-column" : "fn__flex"} fn__flex-1" style="min-height: auto">
        ...
        <div class="fn__flex-1 card__empty">${siyuanI18n.emptyContent}</div>
    </div>
</div>`,
        ...
        dialog.element.addEventListener("click", (event) => {
            ...
            while (target && (dialog.element !== target)) {
                ...
                if (type === "close") {
                    ...
                    confirmDialog(siyuanI18n.reset,
                        siyuanI18n.resetCardTip.replace("${x}", dialog.element.querySelector(".counter").textContent), () => {
                            fetchPost("/api/riff/resetRiffCards", {
                                type: deckType === "" ? "deck" : deckType.toLowerCase(),
                                deckID: deckType === "" ? deckID : Constants.QUICK_DECK_ID,
                                id: deckID,
                    ...
                    fetchPost("/api/riff/removeRiffCards", {
                        ...
                        if (!nextElment) {
                            getArticle(edit, "");
                            listElement.innerHTML = `<div class="b3-list--empty">${siyuanI18n.emptyContent}</div>`;
                        } else {
...
<span data-position="parentE" aria-label="${siyuanI18n.revisionCount}" class="ariaLabel counter${item.riffCard?.reps === 0 ? " fn__none" : ""}">${item.riffCard?.reps}</span>
<span data-position="parentE" aria-label="${siyuanI18n.nextDue}" class="ariaLabel b3-list-item__meta${!item.riffCard?.due ? " fn__none" : ""}">${dayjs(item.riffCard?.due).format("YYYY-MM-DD")}</span>
<span data-position="parentE" data-type="reset" data-id="${item.id}" class="b3-list-item__action ariaLabel" aria-label="${siyuanI18n.reset}">
    <svg><use xlink:href="#iconUndo"></use></svg>
</span>
<span data-position="parentE" data-type="remove" data-id="${item.id}" class="b3-list-item__action b3-list-item__action--warning ariaLabel" aria-label="${siyuanI18n.removeDeck}">
...
<span data-position="parentE" data-type="remove" data-id="${item.id}" class="b3-list-item__action b3-list-item__action--warning ariaLabel" aria-label="${siyuanI18n.removeDeck}">
    ...
    if (blocks.length === 0) {
        listHTML = `<div class="b3-list--empty">${siyuanI18n.emptyContent}</div>`;
    }
    return listHTML;
};
