...
embeddingPending: `待索引：`
embeddingFailed: `失败：`
embeddingIgnoredByLen: `长度忽略：`
embeddingIgnoredByConfig: `配置忽略：`
...
safeModeTip: `🛡️ 安全模式已启用（已禁用代码片段、插件、自定义主题和自定义图标）`
gutterTipBacklink: `点击 <span class='ft__on-surface'>或</span> ⌘/ <span class='ft__on-surface'>打开菜单</span><br>拖拽 \$\{x\} <span class='ft__on-surface'>移动位置</span><br>⌘点击⌥→ <span class='ft__on-surface'>打开</span>`
testConnection: `测试连接`
testConnectionTesting: `测试中...`
testConnectionSuccess: `连接成功`
testConnectionFail: `连接失败`
testConnectionFailModelRequired: `请填写模型名称`
testConnectionFailModelNotFound: `该模型不在可用模型列表中`
fetchAvailableModels: `获取可用模型`
fetchAvailableModelsSuccess: `获取成功`
fetchAvailableModelsFail: `获取可用模型失败`
selectModel: `请选择模型`
secretsVariables: `密钥和变量`
secrets: `密钥`
variables: `变量`
secretsTip: `提供给 Skill 和 MCP 通过 $NAME、\$\{NAME\} 或 {{secrets.NAME\}\} 引用，插件可通过接口使用。密钥值不会进入 AI 对话，并且是加密存储的`
variablesTip: `提供给 Skill 和 MCP 通过 $NAME、\$\{NAME\} 或 {{vars.NAME\}\} 引用，也可在对话和技能正文中使用，插件可通过接口使用。变量值会进入 AI 对话，并且是明文存储的`
secretName: `密钥命名`
variableName: `变量命名`
addSecret: `新增密钥`
addVariable: `新增变量`
secretValue: `密钥值`
variableValue: `变量值`
secretNameTip: `用于引用此密钥的名称，在 Skill 和插件中使用 {{secrets.NAME\}\}`
variableNameTip: `用于引用此变量的名称，在 Skill 和插件中使用 {{vars.NAME\}\}`
secretNameRequired: `密钥名不能为空`
secretNameDuplicate: `已存在同名密钥`
variableNameRequired: `变量名不能为空`
variableNameDuplicate: `已存在同名变量`
noSecretConfigured: `暂无密钥`
noVariableConfigured: `暂无变量`
agentGoToSetting: `前往配置`
kernelFault3: `常见原因与排查`
kernelFault4: `反向代理未转发 WebSocket`
kernelFault5: `若通过 NGINX 等反向代理访问，必须为 <strong>/ws</strong> 单独配置协议升级头，否则 WebSocket 握手会失败。`
kernelFault6: `内核进程已退出或崩溃`
kernelFault7: `请确认容器或服务仍在运行，必要时重启容器，完整日志可在 [设置 - 应用 - 系统日志] 中导出。`
kernelFault8: `请重启思源笔记，完整日志可在 [设置 - 应用 - 系统日志] 中导出。`
kernelFault9: `杀毒软件或防火墙拦截`
kernelFault10: `某些安全软件可能误拦截思源笔记，请将思源笔记安装路径加入杀毒软件与防火墙的信任列表（白名单）后重试。`
tokenUsage: `上下文用量`
tokenCatSystem: `系统提示`
tokenCatSkills: `技能`
tokenCatMessages: `对话消息`
tokenCatNativeToolsDef: `原生工具定义`
tokenCatPluginToolsDef: `插件工具定义`
tokenCatMcpToolsDef: `MCP 工具定义`
tokenCatNativeTool: `原生工具调用`
tokenCatPluginTool: `插件工具调用`
tokenCatMcpTool: `MCP 工具调用`
tokenCatOther: `其他`
tokenCatCached: `缓存命中`
agentNoModel: `尚未配置 AI 模型`
agentNoModelTip: `请先在 [设置 - 人工智能 - API 提供商] 中配置提供商与模型，然后再来和智能体对话。`
dragTip2Heading: `转换为标题`
dragTipRef: `插入引用`
dragTipEmbed: `插入嵌入块`
...
dragTipListItemChild: `移动为 \$\{x\} 的子项`
dragTipListItemBefore: `移动到 \$\{x\} 的上方`
dragTipListItemAfter: `移动到 \$\{x\} 的下方`
dragTip2DocChild: `转换为文档作为 \$\{x\} 的子文档`
dragTip2DocBefore: `转换为文档并移动到 \$\{x\} 的上方`
dragTip2DocAfter: `转换为文档并移动到 \$\{x\} 的下方`
workspace: `工作空间`
addAiProvider: `添加提供商`
addAiModel: `添加模型`
...
aiModelDisplayNameTip: `在模型列表中展示的名称，留空则使用模型名称`
aiEditingModelPickerTip: `选择编辑器块菜单或斜杠菜单 AI 操作使用的模型`
aiAgentModelPickerTip: `选择智能体使用的模型`
defaultModel: `默认模型`
aiMcpServersTip: `为 AI Agent 配置 Model Context Protocol 外部工具服务`
addAiMcpServer: `添加 MCP 服务`
noMcpServerConfigured: `未配置 MCP 服务`
aiMcpServerSettings: `MCP 服务设置`
aiMcpServerName: `名称`
...
aiMcpArgsTip: `每行一个参数`
aiMcpUrlTip: `远程服务地址`
aiMcpHttpHeaders: `HTTP 请求头`
fillJsonObject: `填写 JSON 对象`
aiMcpHeadersInvalid: `HTTP 请求头格式无效，请填写 JSON 对象`
aiMcpServerNameRequired: `请填写 MCP 服务名称`
aiMcpServerNameDuplicate: `MCP 服务名称已存在，请使用其他名称`
convertGroupToFilter: `转换为条件`
filterWhen: `当`
addFilterCondition: `添加筛选条件`
convertToFilterGroup: `转换为条件组`
filterCombinationAnd: `且`
filterCombinationOr: `或`
addFilterGroup: `添加筛选条件分组`
filterMatchAll: `全部满足`
filterMatchAny: `任一满足`
emptyFilterGroup: `空分组`
undoCrossDocConfirm: `本次撤销将影响以下文档，是否继续？`
calcOperatorTemplate: `模板`
...
featurePreview: `💡 该功能处于早期预览阶段，尚不稳定，建议备份数据后再使用`
agentSessionSearch: `搜索会话...`
agentWelcomeGreeting: `嗨，有什么想法？我来帮你搞定`
agentExample1: `搜索最近的会议记录`
agentExample2: `帮我写一篇日记`
agentExample3: `整理一下项目相关的笔记`
agentConfirmDesc: `智能体将执行<em>{category\}</em>变更操作`
agentCatBlock: `内容块`
agentCatDoc: `文档`
agentCatNotebook: `笔记本`
agentCatTag: `标签`
agentCatBookmark: `书签`
agentCatFile: `文件`
agentCatAsset: `资源文件`
agentCatAttr: `属性`
agentCatDailynote: `日记`
agentCatImport: `导入`
agentCatRepo: `数据仓库`
agentCatHistory: `历史`
agentCatSync: `同步`
agentCatDatabase: `数据库`
agentCatDefault: `变更`
agentNotifyConfirm: `思源智能体需要你的确认`
agentNotifyDone: `思源智能体已回复`
agentConfirmApprove: `确认执行`
agentConfirmReject: `拒绝`
...
agentMaxRetries: `最大重试次数`
agentMaxRetriesTip: `API 请求失败时的最大重试次数`
agentMaxToolCallRounds: `最大工具调用轮数`
agentMaxToolCallRoundsTip: `单次会话中工具调用的最大轮数，0 表示不限制`
agentChat: `智能体`
agentQuestionSubmit: `提交`
agentQuestionCustom: `输入自定义答案...`
agentRegenerate: `重新回答`
...
configGroupBehavior: `行为`
configGroupBlockFeatures: `块特性`
configGroupBidirectionalLinks: `双向链接`
configGroupMarkdownInlineSyntax: `Markdown 行级语法`
configGroupAdvanced: `高级`
configGroupTabs: `页签`
configGroupNewDocument: `新建文档`
configGroupFileManagement: `文件管理`
configGroupOthers: `其他`
configGroupContent: `内容`
configGroupInterface: `界面`
configGroupControls: `控制`
configGroupPersonalization: `个性化`
configGroupCardCreation: `制卡`
configGroupReview: `复习`
configGroupReferences: `关联`
configGroupFormat: `格式`
configGroupPDF: `PDF`
configGroupImages: `图片`
configGroupPandoc: `Pandoc`
configGroupAccount: `账号`
accountSync: `账号与同步`
configGroupSync: `同步`
configGroupLocalDataRepo: `本地数据仓库`
authentication: `鉴权`
configGroupServer: `伺服`
configGroupPublish: `发布`
application: `应用`
configGroupData: `数据`
configGroupGeneral: `常规`
configGroupMaintenance: `维护`
switchToFloatingLayout: `切换到浮动布局`
switchToFixedLayout: `切换到固定布局`
hyperlink: `超链接`
copyDoc: `复制全文`
position: `位置`
...
useChrome: `仅支持在 Chrome 浏览器中使用，使用其他浏览器会遇到无法解决的兼容性问题`
...
upgrade: `升级`
downgrade: `降级`
clipboardPermissionDenied: `剪贴板权限被拒绝，请在浏览器地址栏允许访问剪贴板后再试。注意：http 协议可能无法访问剪切板，请使用 Ctrl+V / ⌘V 进行粘贴`
jumpTo: `跳转到`
reloadUI: `刷新界面`
reloadUITip: `关闭插件、关闭 JS 代码片段或切换主题，可能需要刷新界面才能完全生效`
...
exportConf: `导出设置`
exportConfTip: `账号、锁屏密码、同步、API token 和数据仓库密钥不会被导出`
importConf: `导入设置`
...
publishAccess: `发布访问控制`
publishAccessPublic: `公开`
publishAccessProtected: `保护`
publishAccessHidden: `隐藏`
publishAccessPrivate: `私密`
publishAccessForbidden: `禁止`
publishAccessPublicComment: `完全公开访问`
publishAccessProtectedComment: `公众可见，需密码访问`
publishAccessHiddenComment: `公众不可见，可直接访问`
publishAccessPrivateComment: `公众不可见，需密码访问`
publishAccessForbiddenComment: `禁止访问`
copyMirror: `复制镜像`
duplicateMirror: `创建镜像副本`
duplicateCompletely: `创建完整副本`
isMsStoreVerTip: `当前使用的版本为微软商店版，请在微软商店中检查更新`
andSubFile: `确定删除 <b>\$\{x\}</b> 及其 \$\{y\} 个子文档吗？`
confirmDeleteTip: `确定删除 <b>\$\{x\}</b> 吗？`
rollbackTip: `删除后可在 [数据历史] 中恢复，按 [设置 - 文档 - 历史保留天数] 保留 <code class='fn__code'>\$\{x\}</code> 天`
newView: `添加视图`
newCol: `添加字段`
newRow: `添加条目`
enterKey: `回车`
...
includeTime: `具体时间`
accountSupport1: `自 2020 年 8 月 31 日第一次公开发版以来，思源笔记已累计发版超过 880 次，解决用户需求与反馈超过 16000 次，提交代码超过 22000 次，背后无数次的辛勤付出与长期坚持不懈的更新都离不开您的支持与反馈。`
...
cloudConfigTip: `请在 [设置 - 账号与同步 - 云端存储服务提供商] 中进行配置`
confirmUninstall: `确定卸载 \$\{name\} ？<br>卸载可能会删除所有相关配置和数据，请谨慎决定！`
...
attrAliasTip: `为该块设置别名，一个块可以通过英文逗号分隔设置多个别名`
attrMemoTip: `为该块设置备注，仅支持纯文本`
switchDirect: `交换对比方向`
...
apiKey: `API Key`
apiKeyTip: `建议为思源单独分配 API Key 以便管理`
apiTimeout: `超时时间`
...
apiTemperature: `温度`
apiTemperatureTip: `请求 API 时传入的 <code class='fn__code'>temperature</code> 参数，用于控制生成的文本随机性（0~2，值越高越随机）`
...
apiProvider: `API 提供商`
apiProviderTip: `管理 AI 模型提供商及其模型。兼容 Chat Completions API 接口的提供商（如 DeepSeek、Kimi、MiniMax）均可使用`
skip: `跳过`
nextRound: `下一轮`
save: `保存`
ai: `人工智能`
...
uploadData2Cloud: `⬆️ 上传本地数据快照`
uploadData2CloudTip: `将本地数据生成数据快照后上传到云端`
downloadDataFromCloud: `⬇️ 下载云端数据快照`
downloadDataFromCloudTip: `将云端最新数据快照下载后合并到本地数据，冲突文件以云端为准`
targetBlockID: `目标块 ID`
transferBlockRefTip: `将该块的所有引用转移到目标块上`
transferBlockRef: `转移引用`
sortByFiletree: `使用文档排序规则`
tabToWindow: `移动到新窗口`
openByNewWindow: `使用新窗口打开`
exitFocus: `退出聚焦`
...
activationCode: `激活码`
activationCodePlaceholder: `来自特定渠道的订阅激活码。官网购买无需填入，账号权限会自动激活`
...
showHeadingWithBlocks: `显示标题块与下方的块`
showHeadingOnlyTitle: `仅显示标题块`
showHeadingOnlyBlocks: `仅显示标题块下方的块`
...
height: `高度`
deactivateUser: `注销账号`
deactivateUserTip: `<ul class='fn__list'><li>账号一旦停用就无法恢复，请不要随意尝试</li><li>用户名会被 <code>someone101</code> 填充（后面的数字是用户编号），密码会被随机数重置</li><li>清空用户昵称、自我标签、URL、简介、头像、站点连接、邮寄地址等信息</li><li>所有隐私设置开关将恢复默认的公开状态</li><li>解绑关联的第三方社交账号、手机</li><li><strong>用户名、手机号、绑定的第三方账号以后将不能再次被使用</strong></li><li>用户状态被置为停用，禁止登录</li></ul>`
mergeCell: `合并单元格`
cancelMerged: `拆分单元格`
...
use: `使用`
uninstallTip: `当前正在使用中，请在 [设置 - 外观] 中切换后再卸载`
safeQuit: `退出应用`
anchor: `锚文本`
showMore: `显示更多`
refPopover: `在浮窗中打开`
...
stickOpen: `保持全部展开`
cloud: `云端`
setEmojiTip: `请在 [设置 - 外观 - 添加自定义表情] 中进行配置`
openSyncTip1: `启用云端同步`
...
searchHanSensitive: `区分中文简繁体`
searchHanSensitive1: `启用后所有搜索相关功能将区分中文简繁体`
toggleWin: `隐藏/显示窗口`
customSort: `自定义排序`
collapse: `折叠`
blockEmbed: `嵌入块`
...
gutterTip: `点击 <span class='ft__on-surface'>或</span> ⌘/ <span class='ft__on-surface'>打开菜单</span><br>拖拽 \$\{x\} <span class='ft__on-surface'>移动位置</span>`
...
heading6: `六级标题`
unorderedList: `无序列表`
searchSubTypeListTip: `列表子类型（有序/无序/任务）同样适用于列表项`
...
previous: `上一个`
next: `下一个`
kernelFault0: `思源笔记遇到了一点问题...`
kernelFault1: `与内核的连接已断开，但数据通常是安全的。思源会每 3 秒自动尝试重新连接，如果内核恢复，本窗口会自动关闭。`
...
fontSizeScrollZoom: `快速调整编辑器字号`
fontSizeScrollZoomTip: `启用后按住 <code class='fn__code'>Ctrl</code> 通过鼠标滚轮或触摸板双指缩放手势可快速调整字号`
...
insertRight: `在页签右侧打开`
accountUnpaid: `未付费`
accountSubscriptionExpired: `订阅已过期`
account1: `成为付费会员`
account4: `升级为订阅会员`
...
appearance1: `代码块主题`
appearance10: `关闭按钮行为`
appearance11: `最小化到系统托盘`
appearance12: `选择点击窗口关闭按钮后的行为`
appearance16: `隐藏底部状态栏`
appearance17: `启用后将隐藏状态栏`
appearance18: `配置状态栏消息开关`
appearance19: `页签融合至顶栏`
appearance20: `启用后页签栏将与顶栏合并为同一行，以获得更大的编辑区域`
min: `最小化`
restore: `向下还原`
max: `最大化`
md2: `代码块连字`
...
md41: `使用英文状态下的逗号 <code class='fn__code'>,</code> 或换行符进行分隔，逗号自身可通过 <code class='fn__code'>\,</code> 转义。支持使用 <code class='fn__code'>/</code> 包裹正则表达式，例如 <code class='fn__code'>foo,/[0-9]+/</code> 表示排除 foo 和数字`
fileTree2: `在编辑器页签切换时文档面板会自动选中当前的文档`
fileTree3: `删除单个文档时不需要确认`
fileTree4: `启用时删除单个文档不再弹出确认框`
fileTree5: `新建块引用文档的默认存放路径`
fileTree6: `例如 <code class='fn__code'>/folder1/{{now | date "20060102150405"\}\}/</code>，以 <code class='fn__code'>/</code> 开头则使用选中笔记本的根路径，不以 <code class='fn__code'>/</code> 开头则使用当前文档相对路径`
fileTree7: `在当前页签中打开`
fileTree8: `新打开的文档页签将会替换没有修改过的页签`
fileTree9: `启动时关闭所有页签`
fileTree10: `启用后每次启动界面后都会自动关闭上次打开未钉住的页签`
fileTree11: `新建日记`
fileTree12: `新建文档的默认存放路径`
...
fileTree27: `例如 <code class='fn__code'>/闪念速记/{{now | date "20060102150405"\}\}</code>，不使用模板变量时闪念速记将添加到该路径指定文档内容的末尾`
...
mount: `打开笔记本`
newNotebook: `新建笔记本`
fileNameRule: `标题不能包含 / 字符，已经自动替换为全角 ／ 字符`
slogan: `重构你的思维`
showInFolder: `打开文件位置`
search: `搜索`
config: `设置`
...
about13: `API token`
about14: `调用 API 时需要通过该 token 进行鉴权<div class="fn__hr--small"></div>HTTP 请求标头 <code class="fn__code">Authorization: token \$\{token\}</code>`
about17: `设置为 <code class='fn__code'>直接连接</code> 时不启用代理`
about18: `使用非环回地址（<code class="fn__code">127.0.0.1</code>/<code class="fn__code">[::1]</code>）时请先设置 锁屏密码`
desktopMode: `桌面模式`
desktopModeTip: `启用后使用桌面端界面布局`
mobileModeTip: `禁用后使用移动端界面布局`
checkUpdate: `检查更新`
currentVer: `当前版本`
...
spellcheckTip2: `启用后将使用内置的拼写检查功能并在下方显示语言列表，请从列表中选择需要检查的语言`
over: `超过`
preview: `导出预览`
desktop: `桌面`
tablet: `平板`
...
_taskAction: {
  ...
  'task.database.index.fts': `执行重建搜索索引`
  ...
  'task.history.generateFile': `执行生成文件历史`
...
_kernel: {
  ...
  '26': `请先在 [设置 - 账号与同步 - 数据仓库密钥] 中初始化数据仓库密钥`
  '27': `正在上传 [%v]`
  '28': `网络异常，请稍后再试`
  '29': `该功能需要购买 <a target='_blank' href='\$\{accountServer\}/subscribe/siyuan'>[年付订阅]</a> 后才能使用（如果你已经购买，请在 [设置 - 账号与同步 - 账号] 中刷新或者重新登录）`
  '30': `获取云端信息失败`
  '31': `账号鉴权失败，请重新登录账号`
  '32': `删除云端笔记本失败`
  ...
  '46': `安装集市包 [%s] 失败：%s`
  '47': `卸载失败：%s`
  '48': `仅列出前 [%d] 个子文档，如需调整请修改 [设置 - 文档 - 最大列出数量]`
  '49': `请在笔记本设置中指定日记存放路径`
  '50': `正在解析引用 [%s]`
  '51': `正在缓存内容块 [%d]`
  '52': `你正在编辑用户指南，请不要在这里编辑，因为在这里编辑的内容不会被保存，请在你自己创建的笔记本中记录笔记`
  '53': `数据同步尚未启用`
  '54': `正在索引引用关系...`
  '55': `已完成索引 [%d] 篇文档的引用关系`
  ...
  '66': `已创建数据文件 [%s]`
  '67': `上传于 %s，下载于 %s`
  '68': `本地数据已超过试用期云端空间最大存储容量 [%s]，无法继续同步数据，<a target='_blank' href='\$\{accountServer\}/subscribe/siyuan'>订阅</a>后云端存储容量将升级为 [8 GB]`
  '69': `下载完毕`
  '70': `正在处理 [%s]，请稍等...`
  '71': `插入资源文件失败，请重新打开文档`
  ...
  '91': `清理未引用的资源文件完毕，已删除 [%d] 个文件，共释放 [%s] 磁盘空间`
  '92': `正在解析文档 [%s]`
  ...
  '119': `正在下载网络文件 [%s]`
  '120': `下载完毕，一共 [%d] 个文件，共占用 [%s] 磁盘空间`
  '121': `该文档中不存在网络文件`
  '122': `插件带有的侧栏功能需要在桌面端配置（可以使用手机浏览器访问 http://127.0.0.1:6806/stage/build/desktop ，配置完成后请重启思源 APP）`
  '123': `添加/选择云端同步目录后才能启用同步功能`
  '124': `请在 [设置 - 账号与同步 - 启用云端同步] 中开启云端同步`
  '125': `自动同步失败次数过多，请尝试手动触发同步，如果还有问题请<a href="https://ld246.com/article/1649901726096" target="_blank">反馈</a>`
  '126': `书签不能为空`
  '127': `订阅距过期还剩 [%d] 天，过期后官方云端存储数据会被彻底删除。续订请访问<a target='_blank' href='\$\{accountServer\}/subscribe/siyuan'>这里</a>`
  '128': `订阅已经过期，过期后官方云端存储数据会被彻底删除（S3/WebDAV 云端存储数据不受影响），订阅过期两天后不再弹出该提醒，续订请访问<a target='_blank' href='\$\{accountServer\}/subscribe/siyuan'>这里</a>`
  ...
  '132': `SQL 搜索方式下不支持替换操作，请使用其他搜索方式`
  '133': `正在下载更新安装包 [%s]`
  ...
  '149': `已经同步数据快照，耗时 %.2fs`
  '150': `上传/下载文件数 %d/%d<br>&emsp;上传/下载分块数 %d/%d<br>&emsp;发送/接收字节数 %s/%s`
  '151': `请勿包含符号 \ / : * ? &quot; &#039; &lt; &gt; |`
  '152': `上传文件数 %d 上传分块数 %d 发送字节数 %s`
  '153': `下载文件数 %d 下载分块数 %d 接收字节数 %s`
  '154': `云端最多只能支持备份 12 个快照`
  '155': `云端同步目录已经重置为 [main]`
  '156': `锁屏密码验证失败，请<button onclick="window.location.reload()">刷新</button>或者重新打开`
  ...
  '170': `正在上传数据仓库分块 %v/%v`
  '171': `正在上传数据仓库引用 %s`
  '172': `如果你忘记了锁屏密码，请在<a href="https://ld246.com/article/1649901726096" target="_blank">这里</a>寻求帮助`
  '173': `请输入锁屏密码`
  '174': `解锁访问`
  '175': `请输入验证码`
  '176': `思源笔记`
  ...
  '180': `不存在符合条件的内容块`
  '181': `已分享文档到社区，<a href="%s" target="_blank">点击查看</a>`
  '182': `正在分享文档，请稍等...`
  '183': `正在校验索引文档 [%d/%d %s]`
  ...
  '197': `请打开笔记本 [%s] 后再试`
  '198': `这不是一个有效的 Data 压缩包。如果该数据包是从 [文档] 导出的，请从 [文档] 导入`
  ...
  '214': `该功能需要登录账号并购买 <a target='_blank' href='\$\{accountServer\}/one-time-pay/siyuan'>[功能特性]</a> 或者 <a target='_blank' href='\$\{accountServer\}/subscribe/siyuan'>[年付订阅]</a> 后才能使用`
  '215': `无法打开新版本创建的数据库视图，请升级到最新版本后再试`
  '216': `正在重建资源文件内容数据索引，请稍等...`
  '217': `[%d/%d] 已经创建资源文件内容数据索引`
  '218': `检测到数据仓库快照过多，降低了程序启动和数据同步速度，请考虑执行 [设置 - 账号与同步 - 数据仓库清理]`
  '219': `云端存储服务不可用，请稍后再试`
  ...
  '243': `仅列出前 [%d] 个标签（含子标签），如需调整请修改 [设置 - 文档 - 最大列出数量]`
  '244': `注意：请不要在用户指南中编辑，因为在这里编辑的内容不会被保存，请在你自己创建的笔记本中记录笔记`
  '245': ``
  ...
  '252': `🚨 Microsoft Defender 可能会误杀内核、误删数据和严重降低运行性能，建议将思源安装路径和工作空间路径添加到排除列表<span class="fn__space"></span><button id="addMicrosoftDefenderExclusion" class="b3-button">添加</button><span class="fn__space"></span><button id="ignoreAddMicrosoftDefenderExclusion" class="b3-button b3-button--cancel">忽略</button>`
  '253': `正在压缩文件 [%s]，请稍等...`
  ...
  '293': `无法写入文件`
  '294': `激活码不能为空`
  '295': `该功能需要购买 [年付订阅] 后才能使用（如果你已经购买，请在 [设置 - 账号与同步 - 账号] 中刷新或者重新登录）`
  '296': `正在重建搜索索引...`
