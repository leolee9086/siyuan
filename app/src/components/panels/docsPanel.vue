<template>
    <div class="docs-panel fn__flex-1 fn__flex-column" style="overflow: hidden;max-height: 100%;">
        <!-- 搜索框 -->
        <div class="b3-form__icon fn__size200" style="margin: 8px;">
            <svg class="b3-form__icon-icon">
                <use xlink:href="#iconSearch"></use>
            </svg>
            <input ref="searchInput" v-model="searchKey" @compositionend="handleCompositionEnd"
                @input="handleSearchInput" placeholder="搜索" class="b3-text-field fn__block b3-form__icon-input" />
        </div>

        <!-- 最近文档列表 -->
        <div class="fn__flex-column fn__flex-1">
            <ul :style="isWindow() ? 'border-left:0;' : ''" class="b3-list b3-list--background">
                <li v-for="(doc, index) in filteredDocs" :key="doc.rootID" :data-index="index"
                    :data-node-id="doc.rootID" class="b3-list-item" :class="{ 'b3-list-item--focus': index === 0 }"
