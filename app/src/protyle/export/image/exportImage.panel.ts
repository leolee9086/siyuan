...
export const createExportImagePanelContext = (
    ...
    const buttons = containerElement.querySelectorAll<HTMLButtonElement>(".b3-button");
    const cancelButton = buttons.item(0);
    const confirmButton = buttons.item(1);

    // 关键节点缺失时共享 panel 无法安全工作，必须中止并统一报错。
    if (!previewElement || !contentElement || !exportImageElement || !watermarkPreviewElement || !keepFoldElement || !watermarkElement || !ratioElement || !cancelButton || !confirmButton) {
