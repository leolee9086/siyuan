 ...
 * 意图：统一看板列头渲染逻辑，避免调用方分散处理不同分组值类型。
 ...
 */
export const getKanbanTitleHTML = (group: IAVView, counter: number) => {
    ...
    if (["mSelect", "select"].includes(groupType)) {
        ...
        nameHTML = selectedItems.map((item) => `<span class="b3-chip" style="background-color:var(--b3-font-background${item.color});color:var(--b3-font-color${item.color})">${escapeHtml(item.content)}</span>`).join("");
    }
