...
const genAVRollupHTML = (value: IAVCellValue) => {
    ...
    switch (value.type) {
        case "block":
            if (value?.isDetached) {
                html = `<span>${escapeHtml(value.block?.content || siyuanI18n.untitled)}</span>`;
            } else {
                html = `<span data-type="block-ref" data-id="${value.block.id}" data-subtype="s" class="av__celltext--ref">${escapeHtml(value.block?.content || siyuanI18n.untitled)}</span>`;
            }
            break;
        case "text":
...
<span class="fn__space"></span><span class="fn__flex-center ft__on-surface b3-tooltips__w b3-tooltips" aria-label="${siyuanI18n.format}">${value.number.formattedContent}</span><span class="fn__space"></span>`;
        ...
        case "updated":
            ...
            break;
        case "url":
            html = `<input value="${escapeAttr(value.url.content)}" class="b3-text-field b3-text-field--text fn__flex-1" placeholder="${siyuanI18n.empty}">
<span class="fn__space"></span>
<a ${value.url.content ? `href="${escapeAttr(value.url.content)}"` : ""} target="_blank" aria-label="${siyuanI18n.openBy}" class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips"><svg><use xlink:href="#iconLink"></use></svg></a>`;
            break;
        case "phone":
            html = `<input value="${escapeAttr(value.phone.content)}" class="b3-text-field b3-text-field--text fn__flex-1" placeholder="${siyuanI18n.empty}">
<span class="fn__space"></span>
<a ${value.phone.content ? `href="tel:${escapeAttr(value.phone.content)}"` : ""} target="_blank" aria-label="${siyuanI18n.openBy}" class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips"><svg><use xlink:href="#iconPhone"></use></svg></a>`;
            break;
        case "checkbox":
...
<span class="fn__space"></span>
<a ${value.email.content ? `href="mailto:${escapeAttr(value.email.content)}"` : ""} target="_blank" aria-label="${siyuanI18n.openBy}" class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips"><svg><use xlink:href="#iconEmail"></use></svg></a>`;
            break;
        case "relation":
