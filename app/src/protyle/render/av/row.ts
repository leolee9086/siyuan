...
import {cellValueIsEmpty, genCellValue} from "./cell.value";
...
import {getAvBodyData, getAVSelectStat, resetAVRowSelect, updateAVRowSelect} from "./virtualScroll/state";
import {getSelectedItemInfos} from "./virtualScroll/itemInfo.port";
...
import {setStickyRow} from "./stickyRow.port";

/** 作用：读取已注册的 Gallery/Kanban 插入动画。意图：行事务不加载高层 Gallery owner，且 HMR 期间共享同一 Symbol 槽。调用时机：插入 Gallery 或 Kanban 条目时。 */
const getGalleryInsertionVisualAnimation = () => {
    const visualEffects = Reflect.get(globalThis, Symbol.for("sforge.protyle.av.galleryInsertionVisualEffects"));
    if (!visualEffects || typeof visualEffects !== "object") {
        return;
    }
    const animation = Reflect.get(visualEffects, "insertGalleryItemAnimation");
    ...
    if (options.type === "gallery") {
        ...
        if (galleryData.coverFrom !== 0) {
            ...
            if (galleryRow.coverURL) {
                ...
                html += getCardCoverHTML(coverClass, galleryRow.coverURL, compressedURL,
        ...
        for (const [fieldsIndex, cell] of galleryRow.values.entries()) {
            ...
            const renderedCell = await renderCell(cell.value, options.rowIndex, galleryData.showIcon, "gallery");
    ...
    if (options.type === "kanban") {
        ...
        if (kanbanData.coverFrom !== 0) {
            ...
            if (kanbanRow.coverURL) {
                ...
                html += getCardCoverHTML(coverClass, kanbanRow.coverURL, compressedURL,
        ...
        for (const [fieldsIndex, cell] of kanbanRow.values.entries()) {
            ...
            const fullRowClass = kanbanData.fields[fieldsIndex].type === "block" ||
                ...
                " av__gallery-field--full-row" : "";
            const renderedCell = await renderCell(cell.value, options.rowIndex, kanbanData.showIcon, "kanban");
...
${cell.color ? `color:${cell.color};` : ""}">${renderedCell}</div>`;
    ...
    for (const [index, cell] of tableRow.cells.entries()) {
        ...
        const renderedCell = await renderCell(cell.value, options.rowIndex, tableData.showIcon);
 ...
 */
export const insertAttrViewBlockAnimation = async (options: {
    protyle: IProtyle,
    ...
    const cellElements = Array.from(previousElement.querySelectorAll(".av__cell")) as HTMLElement[];
    for (const [index, item] of cellElements.entries()) {
        let lineNumber = 1;
        ...
        const renderedCell = await renderCell(genCellValue(colType, null), lineNumber);
...
${colType === "block" ? ' data-detached="true"' : ""}>${renderedCell}</div>`;
    ...
    fetchPost("/api/av/getAttributeViewAddingBlockDefaultValues", {
        ...
        if (response.data.values) {
            ...
            for (const rowItem of Array.from(options.blockElement.querySelectorAll('[data-type="ghost"]'))) {
                for (const cellItem of Array.from(rowItem.querySelectorAll(".av__cell")) as HTMLElement[]) {
                    ...
                    if (updateIds.includes(cellItem.dataset.colId)) {
                        ...
                        cellItem.innerHTML = await renderCell(cellValue);
...
export const deleteRow = (blockElement: HTMLElement, protyle: IProtyle) => {
    const selectedItems = getSelectedItemInfos(blockElement);
...
export const insertRows = (options: {
    ...
    const shouldRenderGalleryItem = ["gallery", "kanban"].includes(options.blockElement.getAttribute("data-av-type"));
    // 卡片视图需要先插入占位条目，再由服务端返回默认字段值填充。
    if (shouldRenderGalleryItem) {
        const insertGalleryItemAnimation = getGalleryInsertionVisualAnimation();
    ...
    // 表格视图继续使用既有行占位动画，不经过 Gallery 视觉端口。
    if (!shouldRenderGalleryItem) {
...
// 行模块装配后发布固定行实现，供内核响应层按需调用。
setStickyRow(stickyRow);
