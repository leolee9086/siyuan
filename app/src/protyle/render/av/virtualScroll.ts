...
import type {AVVirtualBodyState} from "./virtualScroll/virtualScroll.types";
...
import {setSelectedItemInfos} from "./virtualScroll/itemInfo.port";
import type {IAVItemInfo} from "./virtualScroll/itemInfo.types";

export type {IAVItemInfo} from "./virtualScroll/itemInfo.types";

// 选择快照操作的唯一实现位于统一状态模块，这里从主入口再导出以兼容两条导入路径。
export {updateAVRowSelect, resetAVRowSelect, getAVSelectStat} from "./virtualScroll/state";

const BUFFER_RATIO = 1;

// 分组表格需要底部占位保持未渲染行的布局高度，在统一 body 状态上扩展该字段。
interface IBodyState extends AVVirtualBodyState {
    // 仅分组表格使用底部占位保持未渲染行的布局高度。
    reserveBottomSpacer?: boolean;
}

// 各 AV block 的最新数据快照（含非虚拟滚动场景），供选中项统计与数据读取接口回退使用。
...
const doTrim = async (blockElement: HTMLElement, elementRect: DOMRect): Promise<void> => {
    ...
    // AV 重渲/新增分组/局部更新未走完整 initVirtualScroll 时数据源可能缺失，跳过本次 trim，
    ...
    for (const bodyEl of Array.from(bodies)) {
        const state = getAVVirtualBodyState(bodyEl) as IBodyState | undefined;
        // body 尚未在 initVirtualScroll 中登记（重渲/新增分组/局部更新未走完整流程），
        // 状态查不到则跳过本次 trim，避免解引用 undefined.view
        if (!state) {
            continue;
        }
        ...
        try {
            ...
            const rebuildTableWindow = async (renderWindow: ReturnType<typeof getGroupTableViewportWindow>) => {
                if (!renderWindow) {
                    return;
                }
            ...
            if (groupedTable && (spacerElement || bottomSpacerElement) && windowOutsideBuffer && bodyIntersectsViewport) {
                ...
                await rebuildTableWindow(getGroupTableViewportWindow({
                    dataStart,
                    dataEnd,
...
const getBodyData = (bodyEl: HTMLElement) => {
    ...
    if (!avEl) return null;
    const stored = getAVVirtualDataSource(avEl);
    const data = blockDataStore.get(avEl) || stored?.data;
    if (!data) return null;

    const groupId = bodyEl.dataset.groupId;
    return groupId ? data.view.groups.find((g: IAVView) => g.id === groupId) ?? null : data.view;
};
...
export const ensureAVTableAdjacentRow = (rowElement: HTMLElement, direction: "previous" | "next") => {
    ...
    const state = getAVVirtualBodyState(bodyElement) as IBodyState | undefined;
    ...
    setAVVirtualBodyState(bodyElement, state);
...
// 虚拟滚动状态装配后发布只读选中信息，避免行渲染加载整个滚动实现。
setSelectedItemInfos(getAVSelectedItemInfos);
...
export const trimAVRows = (blockElement: HTMLElement, elementRect: DOMRect): void => {
    ...
    requestAnimationFrame(() => {
        ...
        void doTrim(blockElement, elementRect);
    });
};

// 直接触发 doTrim（不另起 rAF），供已处于 rAF 回调中的调用方使用，例如 scroll 事件中
...
export const initVirtualScroll = (options: {
    ...
    const storeKey = String(options.blockElement.getAttribute("data-av-id")) +
        String(options.blockElement.getAttribute(Constants.CUSTOM_SY_AV_VIEW));
    if (virtualized) {
        registerAVVirtualDataSource(options.blockElement, options.protyle, options.data);
    } else {
        // 与登记逻辑对称：视图退出虚拟滚动后清除旧数据源，避免数据读取接口命中过期数据。
        getAVVirtualScrollRegistry().dataSources.delete(storeKey);
    }
        ...
        if (options.data.viewType === "table") {
            ...
            if (!firstRow || !lastRow) {
                if (needsGroupedTableState) {
                    ...
                    setAVVirtualBodyState(item, emptyState);
                }
                return;
            }
            ...
            setAVVirtualBodyState(item, state);
