...
import {escapeAttr, escapeHtml, escapeLessThans} from "../../../util/DOM/escape";
import {submitAVRelationTransaction} from "../../wysiwyg/transaction/prepared/av/avRelation";
import {transaction} from "../../wysiwyg/transaction/submit";
import {getAttrViewCellValueText} from "./cell/valueText/port";
...
const genSearchList = (element: Element, keyword: string, options: IOpenSearchAVOptions, signal: AbortSignal,
    ...
    <svg aria-label="${siyuanI18n.thisDatabase}" style="margin: 0 0 0 4px" class="b3-list-item__hinticon ariaLabel${item.avID === options.avID ? "" : " fn__none"}"><use xlink:href="#iconInfo"></use></svg>
</div>`;
            if (hasChildren) {
...
const getRelationGridTemplate = (columns: IAVColumn[], rows: IAVRow[],
    ...
    } as IAVTable, getAttrViewCellValueText(), measureText);
...
style="${cell?.bgColor ? `background-color:${cell.bgColor};` : ""}${cell?.color ? `color:${cell.color};` : ""}">${cell?.value ?
                renderCell(cell.value, 0, false, "table") : ""}</span>`;
        }
    });
    return html + "</div>";
};
...
${genRelationLoaderHTML(state.loading, state.loaderVisible)}`;
    ...
    listElement.addEventListener("click", (event) => {
        ...
        event.preventDefault();
        event.stopPropagation();
        options.protyle.app.openDatabaseRow(options.protyle, {
