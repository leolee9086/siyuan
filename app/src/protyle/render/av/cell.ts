...
import {setAttrViewCellOtherElementsSync} from "./cell/sync/port";
import {setAttrViewCellUpdate} from "./cell/update/port";
import type {IAttrViewCellUpdateOptions} from "./cell/update/types";
import {setAttrViewCellValueText} from "./cell/valueText/port";
...
// cell 组合层拥有具体 DOM 同步实现，并在模块装配时向低层资产和选择编辑路径发布稳定端口。
setAttrViewCellOtherElementsSync(updateAttrViewCellInOtherElements);

/**
 * 作用：把 cell 更新实现适配为低层对象参数端口。
 * 意图：上传和粘贴可复用完整更新语义，同时不导入 cell 聚合模块。
 * 调用时机：cell 模块装配端口时。
 * @同步豁免: 生命周期 - 当前更新事务需要同步保留所有可选开关。
 */
setAttrViewCellUpdate(async (options: IAttrViewCellUpdateOptions) => {
    const result = await updateCellsValue(
        options.protyle,
        options.nodeElement,
        options.value,
        ...
        options.forceOperation,
        options.replaceSelectValues,
        options.stableCells,
        options.updateElements,
    );
    if ("doOperations" in result) {
        return result;
    }
    return {
        ...result,
...
// 注册需使用已初始化的文本测量函数，避免模块初始化期间读取未赋值的 const。
setAttrViewCellValueText(getCellValueText);
