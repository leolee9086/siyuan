...
import {isInMobileApp, updateHotkeyTip} from "../../util/compatibility";
...
import {applyAVRenderContext, failAVRender, persistAVLocateView, beginAVRender, getAVLocateParams, isCurrentAVRender, setAVLocateRequest} from "./locate/state/state";
...
import {setAVLocateRenderer} from "./locate/renderer.port";
// 以下符号在上游位于 ./col 与 ./view，本分叉已拆分为子模块，按本仓库结构取用
...
// getGroupTitleHTML 由本文件内的上游版本定义并导出（原先自 ./render.table 再导出，避免重复导出冲突）
...
// AV 根渲染模块装配后发布实现，供事务和响应层按需调用。
setAVLocateRenderer(avRender);
