...
export function 整理零宽空格(
    ...
    for (let i = 0; i <= newNodes.length; i++) {
        const previousElement = 获取前一个元素(i, newNodes, hasPreviousSibling);

        let currentNode: Node | undefined = newNodes[i];
        const prevSibling = newNodes[i - 1];

        if (!currentNode && prevSibling) {
            const next = hasNextSibling(prevSibling);
 ...
 * 处理文本节点的ZWSP
 */
function 处理文本节点ZWSP(currentNode: Node, previousElement: Node | undefined): void {
    ...
    const previousType = (previousElement && isHTMLElement(previousElement)) ? (previousElement.getAttribute("data-type") || "").split(" ") : [];
 ...
 * 处理元素节点的ZWSP
 */
function 处理元素节点ZWSP(currentNode: Node, previousElement: Node | undefined): void {
    let currentType: string[] = [];
    if (isHTMLElement(currentNode)) {
        currentType = (currentNode.getAttribute("data-type") || "").split(" ");
    }

    const 是特殊类型 = currentType.includes("code") || currentType.includes("tag") || currentType.includes("kbd");

    // 处理 code/tag/kbd 类型：确保开头有 ZWSP
    if (是特殊类型 && !currentNode.textContent?.startsWith(Constants.ZWSP) && isHTMLElement(currentNode)) {
    ...
    const previousType = (previousElement && isHTMLElement(previousElement))
        ? (previousElement.getAttribute("data-type") || "").split(" ")
        : [];
    if ((previousType.includes("code") || previousType.includes("tag") || previousType.includes("kbd")) && isHTMLElement(currentNode)) {
        currentNode.insertAdjacentText("beforebegin", Constants.ZWSP);
    }
}
 ...
 */
function 处理尾部ZWSP(previousElement: Node | undefined): void {
    const currentType = (previousElement && isHTMLElement(previousElement))
        ? (previousElement.getAttribute("data-type") || "").split(" ")
        : [];
 ...
 * 获取前一个元素
 */
function 获取前一个元素(i: number, newNodes: Node[], hasPreviousSibling: (node: Node) => Node | false): Node | undefined {
    ...
    const prev = hasPreviousSibling(nodeAtIndex);
