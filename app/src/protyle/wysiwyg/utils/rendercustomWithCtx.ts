 ...
 * - 问题/改进：
 *   1. 函数名应遵循驼峰命名法 (renderCustomWithCtx)。
 *   2. 存在硬编码的属性黑名单，建议提取为常量。
 */
export const renderCustomWithCtx = (ctx: { ial: IObject, wysiwyg: WYSIWYG }) => {
    ...
    for (const key of ialKeys) {
        const value = ial[key];
        if (value !== undefined && !["title-img", "title", "updated", "icon", "id", "type", "class", "spellcheck", "contenteditable", "data-doc-type", "style", "data-realwidth", "data-readonly", "av-names"].includes(key)) {
 ...
 * - 调用时机：在 input 事件中调用。
 * - 问题/改进：
 *   1. 逻辑较为复杂，包含多个特定的 bug fix，依赖于 dom 结构判断。
 *   2. 依赖全局的 Constants 和工具函数。
 */
export const escapeInline = (protyle: IProtyle, range: Range, event: InputEvent) => {
    ...
    if (event.inputType === "insertLineBreak") {
        handleLineBreak(range, inlineElement, currentTypes);
        return;
    }

    const inputData = event.data || "";
    if (handleInlineStart(inputData, range, inlineElement, currentTypes)) {
        return;
    }

    handleInlineInsertion(inputData, protyle, range, inlineElement, currentTypes);
};
 ...
 * - 调用时机：escapeInline 中检测到 insertLineBreak 时。
 * - 问题/改进：逻辑依赖于特定的 DOM 状态（tagName === "SPAN", textContent startsWith "\n"）。
 */
const handleLineBreak = (range: Range, inlineElement: HTMLElement | null, currentTypes: string[]) => {
 ...
 * - 调用时机：escapeInline 中。
 */
const handleInlineStart = (inputData: string, range: Range, inlineElement: HTMLElement | null, currentTypes: string[]) => {
    ...
    if (currentTypes.length > 0 && range.toString() === "" && range.startOffset === inputData.length &&
        ...
        const dataLength = getDataLength(inputData);
 ...
 * - 意图：修复公式消失等问题。
 * - 调用时机：escapeInline 中。
 * - 问题/改进：条件非常复杂，这就是所谓 "Magic conditions"。
 */
const handleInlineInsertion = (inputData: string, protyle: IProtyle, range: Range, inlineElement: HTMLElement | null, currentTypes: string[]) => {
    if (// 表格行内公式之前无法插入文字 https://github.com/siyuan-note/siyuan/issues/3908
        ...
        executeInlineInsertion(inputData, protyle, range, inlineElement);
    }
};
 ...
 * - 意图：解决嵌套 if 问题。
 * - 调用时机：handleInlineInsertion 条件满足时调用。
 */
const executeInlineInsertion = (inputData: string, protyle: IProtyle, range: Range, inlineElement: HTMLElement | null) => {
 ...
 * - 问题/改进：无
 */
const getDataLength = (inputData: string) => {
