...
export const keydown = (protyle: IProtyle, editorElement: HTMLElement) => {
    editorElement.addEventListener("keydown", async (event: KeyboardEvent & { target: HTMLElement }) => {
        ...
        if (event.shiftKey && event.key.indexOf("Arrow") > -1) {
            // 闃叉杩炵画閫変腑鐨勬椂鍊欐姈鍔?https://github.com/siyuan-note/insider/issues/657#issuecomment-851391217
        } else if (!event.repeat &&
            event.code !== "") { // 鎮诞宸ュ叿浼氳Е鍙戜絾 code 涓虹┖ https://github.com/siyuan-note/siyuan/issues/6573
        ...
        if (!matchHotKey("鈱楥", event) && endElement && nodeElement !== endElement) {
            event.stopPropagation();
            event.preventDefault();
            return;
        }
        ...
        if (!["鈱?, "鈬?, "鈱?, "鈱?].includes(Constants.KEYCODELIST[event.keyCode])) {
            if (Constants.KEYCODELIST[event.keyCode] === "/" ||
                ...
                // 寰疯
                event.key === "\\" ||
                // Mac 鏃ユ枃-缃楅┈瀛?https://github.com/siyuan-note/siyuan/issues/13725
                (event.key === "," && event.keyCode === 229) ||
                // windows 涓枃
                (event.code === "Backslash" && event.key === "Process" && event.keyCode === 229)) {
                protyle.hint.enableSlash = false;
                hideElements(["hint"], protyle);
                // 姝ゅ涓嶈兘杩斿洖锛屽惁鍒欐棤娉曟挙閿€ https://github.com/siyuan-note/siyuan/issues/2795
            }
        }
        // 鏈夊彲鑳借緭鍏?shift+. 锛屽洜姝ら渶瑕佷娇鐢?event.key 鏉ヨ繘琛屽垽鏂?        if (event.key !== "PageUp" && event.key !== "PageDown" && event.key !== "Home" && event.key !== "End" && event.key.indexOf("Arrow") === -1 &&
            ...
            // 寰蒋鍙屾嫾浣跨敤 compositionstart锛屽惁鍒?focusByRange 瀵艰嚧鏃犳硶杈撳叆鏂囧瓧
            event.key !== "Process") {
        ...
        if (!window.siyuan.menus.menu.element.classList.contains("fn__none") &&
            (["鈫?, "鈫?, "鈫?, "鈫?].includes(Constants.KEYCODELIST[event.keyCode]) || Constants.KEYCODELIST[event.keyCode] === "鈫?) &&
        ...
        if ((event.shiftKey && !event.altKey && isNotCtrl(event) && (event.key === "Home" || event.key === "End") && isMac()) ||
            ...
            if (topElement) {
                // 瓒呯骇鍧楀唴宸查€変腑鏌愪釜鍧?                topElement.querySelectorAll(".protyle-wysiwyg--select").forEach(item => {
        ...
        // 鍚戜笂/涓嬫粴鍔ㄤ竴灞?        if (!event.altKey && !event.shiftKey && isNotCtrl(event) && (event.key === "PageUp" || event.key === "PageDown")) {
            if (event.key === "PageUp") {
        ...
        if (!event.altKey && !event.shiftKey && isNotCtrl(event) && !event.isComposing && (event.key.indexOf("Arrow") > -1)) {
            // 闇€浣跨敤 editabled锛屽惁鍒欎唬鐮佸潡浼氭妸璇█瀛楁暟绠楀叆
            ...
            if (nodeElement.classList.contains("code-block") && position.end === nodeEditableElement.innerText.length) {
                // 浠ｇ爜鍧楁崲鏈€鍚庝竴涓?/n 鑲夌溂鏄棤娉曞尯鍒嗘槸鍚﹀湪鍏跺悗鐨勶紝鍥犳缁熶竴鍦ㄤ箣鍓?                position.end -= 1;

            }
            // 闇€浣跨敤 innerText 鍚﹀垯琛ㄦ牸鍐?br 鏃犳硶浼犲敜涓?/n
            if (event.key === "ArrowDown" && nodeEditableElement?.innerText.trimRight().substr(position.start).indexOf("\n") === -1 && (
                ...
                if ((
                        !getPreviousBlock(nodeElement) &&  // 鍒楄〃绗竴涓潡涓哄祵鍏ュ潡锛岀浜屼釜鍧椾负娈佃惤鍧楋紝涓婇敭搴旈€変腑绗竴涓潡 https://ld246.com/article/1652667912155
                        nodeElement.contains(firstEditElement)
                    ) ||
                    (!firstEditElement && nodeElement === protyle.wysiwyg.element.firstElementChild)) {
                    // 涓嶈兘鐢╘n鍒ゆ柇锛屽惁鍒欐枃瀛楄繃闀挎姌琛屽皢閿欒 https://github.com/siyuan-note/siyuan/issues/6156
                    ...
                    if (((nodeEditableElement?.innerText.substr(0, position.end).indexOf("\n") === -1 || position.start === 0) &&
                        ...
                        if (previousElement) {
                            ...
                            if (previousElement) {
                                ...
                                // 浠ｇ爜鍧楁垨浠ヨ蒋鎹㈣缁撳熬鐨勫潡绉诲姩鍏夋爣 鈫?浼氳烦杩?https://github.com/siyuan-note/siyuan/issues/5498
                                // 浠ｇ爜鍧楀叏閫夊悗 鈫?鍏夋爣涓嶄細涓婄Щ https://github.com/siyuan-note/siyuan/issues/11581
                                // 娈佃惤鍧椾笉鑳借缃紝鍚﹀垯 鈫?鍚庡厜鏍囦綅缃笉鑳戒繚鎸?https://github.com/siyuan-note/siyuan/issues/12710
                                if (!foldElement && previousElement.classList.contains("code-block")) {
                                    ...
                                    // 閬囧埌鎶樺彔鍧?                                    foldElement.scrollTop = 0;
                                    focusBlock(foldElement, undefined, true);
                                    scrollCenter(protyle, foldElement);
                                    event.stopPropagation();
                                    event.preventDefault();
                                } else {
                                    // 淇鍏夋爣涓婄Щ鑷?\n 缁撳熬鐨勫潡鏃惰惤鐐归敊璇?https://github.com/siyuan-note/siyuan/issues/14443
                ...
                // 琛ㄦ牸鏃犳硶鍙崇Щ鍔?https://ld246.com/article/1631434502215
                !hasClosestByTag(range.startContainer, "TD") && !hasClosestByTag(range.startContainer, "TH")) {
                // 椤甸潰鎸夊悜涓?鍙崇澶翠涪澶辩劍鐐?https://ld246.com/article/1629954026096
                const lastEditElement = getContenteditableElement(nodeElement);
                // 浠ｇ爜鍧楅渶鏇挎崲鏈€鍚庝竴涓?/n  https://github.com/siyuan-note/siyuan/issues/3221
                ...
                // 椤甸潰鍚戝乏绠ご涓㈠け鐒︾偣 https://github.com/siyuan-note/siyuan/issues/2768
            ...
            if (event.key === "ArrowDown") {
                ...
                if (foldElement) {
                    ...
                    // 闇€浣跨敤 innerText锛屽惁鍒?td 涓殑 br 鏃犳硶杞崲涓?\n; position.end 涓嶈兘鍔?锛屽惁鍒欏€掓暟绗簩琛岃鏈棤娉曚笅绉?                    range.collapse(false);
        ...
        // 鍒犻櫎锛屼笉鍙娇鐢?isNotCtrl(event)锛屽惁鍒欒蒋鍒犻櫎鍥炲鑷?https://github.com/siyuan-note/siyuan/issues/5607
        // 涓嶅彲浣跨敤 !event.shiftKey锛屽惁鍒?https://ld246.com/article/1666434796806
        if ((!event.altKey && (event.key === "Backspace" || event.key === "Delete")) ||
            matchHotKey("鈱僁", event)) {
            ...
            if (imgSelectElement) {
                ...
                if (event.key === "Delete" || matchHotKey("鈱僁", event)) {
                    if (range.startOffset === 0 && range.startContainer.textContent.length === 1) {
                        // 鍥剧墖鍚庝负绌烘牸锛屽湪绌烘牸鍚庡垹闄?https://github.com/siyuan-note/siyuan/issues/13949
                        ...
                        // 鍥剧墖鍓嶆湁涓€涓瓧绗︼紝鍦ㄥ瓧绗﹀悗鍒犻櫎 https://github.com/siyuan-note/siyuan/issues/15911
                        if (position.start === 0 &&
                            range.startContainer.textContent !== Constants.ZWSP &&  // 濡傛灉涓?zwsp 闇€鍓嶇Щ鍏夋爣
                    ...
                    // 闇€浣跨敤 innerText锛屽惁鍒?br 鏃犳硶浼犲敜涓?/n https://github.com/siyuan-note/siyuan/issues/12066
                    // 娈垫湯鍙嶅悜鍒犻櫎 https://github.com/siyuan-note/insider/issues/274
                    if (isEndOfBlock(range) || editElement.textContent.substring(position.start) === "\n") {
                        ...
                        if (nextElement) {
                            ...
                            if (nextRange) {
                                ...
                                if (nextBlockElement &&
                                    ...
                                    // 鍙嶅悜鍒犻櫎鍚堝苟涓轰竴涓潡鏃讹紝鍏夋爣搴斾繚鎸佸湪灏鹃儴 https://github.com/siyuan-note/siyuan/issues/14290#issuecomment-2849810529
                        ...
                        // 鍥剧墖鍓?Delete 鏃犳晥 https://github.com/siyuan-note/siyuan/issues/11209
                        ...
                        if (nextSibling) {
                            ...
                            if (nextSibling.nodeType === 1 && nextSibling.classList.contains("img")) {
                                // 鍏夋爣闇€鍦ㄥ浘鐗囧墠 https://github.com/siyuan-note/siyuan/issues/12452
                    ...
                    if (position.start === 0 && (
                        ...
                        (currentNode && currentNode.nodeType === 3 && !hasPreviousSibling(currentNode) &&
                            // 闇€浣跨敤 textContent锛屾枃鏈厓绱犳病鏈?innerText
                    ...
                    // 鍥剧墖鍚庝负 br锛屽湪 br 鍚庡垹闄?https://github.com/siyuan-note/siyuan/issues/4963
                    ...
                    const rangeNextElement = hasNextSibling(range.startContainer) as HTMLElement;
                    // \n1`2` 1鍚庢寜 Backspace 鍏夋爣閿欒 https://github.com/siyuan-note/siyuan/issues/15424
                    ...
                    // 浠ｇ爜鍧椾腑绌鸿 鈱?Del 寮傚父 https://ld246.com/article/1663166544901
                    ...
                    if (position.start === 2 && inlineElement &&
                        ...
                        // 7.1 ctrl+g 鍚庡垹闄?https://github.com/siyuan-note/siyuan/issues/14290#issuecomment-2867478746
                        inlineElement.innerText !== Constants.ZWSP &&
                        // 闇€鎺掗櫎琛屽唴浠ｇ爜鍓嶆湁涓€涓瓧绗︾殑鎯呭喌
                ...
                // 绌轰唬鐮佸潡鍏ㄩ€夊垹闄ゅ紓甯?https://github.com/siyuan-note/siyuan/issues/6706
                range.collapse(true);
                event.stopPropagation();
                event.preventDefault();
                return;
        ...
        // 杞崲琛?        if (matchHotKey("鈬р啯", event) && selectText === "" && softEnter(range, nodeElement, protyle)) {
            event.stopPropagation();
            event.preventDefault();
            return;
        }

        // 浠ｇ爜鍧楄瑷€閫夋嫨 https://github.com/siyuan-note/siyuan/issues/14126
        if (matchHotKey("鈱モ啯", event) && selectText === "") {
            const selectElements = Array.from(protyle.wysiwyg.element.querySelectorAll(".protyle-wysiwyg--select"));
            if (selectElements.length === 0) {
                selectElements.push(nodeElement);
            }
            if (selectElements.length > 0 && !isIncludesHotKey("鈱モ啯")) {
                ...
                if (languageElements.length > 0) {
                    ...
                    // 鍑芥暟鍐呴儴宸插鐞?                } else if (calloutElements.length > 0) {
        ...
        // 鍥炶溅
        if (matchHotKey("鈫?, event) ||
            (matchHotKey("鈬р啯", event) && nodeType === "NodeHeading")) {
        ...
        if (matchHotKey(window.siyuan.config.keymap.editor.general.copyText.custom, event)) {
            // 鐢ㄤ簬鏍囪瘑澶嶅埗鏂囨湰 *
            if (selectText !== "") {
                // 鍜屽鍒跺潡寮曠敤淇濇寔涓€鑷?https://github.com/siyuan-note/siyuan/issues/9093
        ...
        if (isNewNameFile || matchHotKey(window.siyuan.config.keymap.editor.general.newNameSettingFile.custom, event)) {
            if (!selectText.trim() && (nodeElement.querySelector("tr") || nodeElement.querySelector("span"))) {
                // 娌￠€変腑鏃讹紝閮芥槸绾枃鏈氨鍒涘缓瀛愭枃妗?https://ld246.com/article/1663073488381/comment/1664804353295#comments
            } else {
                if (!selectText.trim() &&
        ...
        if (event.key === "Escape") {
            if (event.repeat) {
                ...
                if (!protyle.toolbar.element.classList.contains("fn__none") ||
                    ...
                    // 闃叉 ESC 鏃堕€変腑褰撳墠鍧?                    window.siyuan.menus.menu.remove(true);
        ...
        if (matchHotKey(window.siyuan.config.keymap.editor.insert.code.custom, event) &&
            ...
            if (editElement) {
                ...
                // 闇€瑕?EscapeHTMLStr https://github.com/siyuan-note/siyuan/issues/11451
        ...
        if (!nodeElement.classList.contains("code-block") && !event.repeat && !isInEmbedBlock(nodeElement)) {
            ...
            protyle.options.toolbar.find((menuItem: IMenuItem) => {
                ...
                if (matchHotKey(menuItem.hotkey, event)) {
                    // 璁剧疆 lastHTMLs 浼氬鑷? protyle.toolbar.range 鍜?range 涓嶄竴鑷达紝闇€閲嶇疆涓€涓?https://github.com/siyuan-note/siyuan/issues/10933
                    protyle.toolbar.range = range;
        ...
        if (event.key === "Tab" && isNotCtrl(event) && !event.altKey) {
            ...
            if (nodeType === "NodeCodeBlock" && selectText !== "") {
                ...
                // 璇█浼樺厛绾у鐞?https://github.com/siyuan-note/siyuan/issues/14767
        ...
        if (refElement) {
            ...
            if (matchHotKey(window.siyuan.config.keymap.editor.general.openBy.custom, event)) {
                ...
                // 鎵撳紑鍧楀紩鍜岀紪杈戝櫒涓紩鐢ㄣ€佸弽閾俱€佷功绛句腑鐐瑰嚮浜嬩欢闇€淇濇寔涓€鑷达紝閮藉姞杞戒笂涓嬫枃
                checkFold(id, (zoomIn) => {
                    openFileById({
                        app: protyle.app,
                        id,
        ...
        // 鍜岃嚜瀹氫箟 alt+shift+宸?鍙?鍐茬獊锛岄檷浣庝紭鍏堢骇  https://github.com/siyuan-note/siyuan/issues/14638
        if (event.shiftKey && (event.key === "ArrowLeft" || event.key === "ArrowRight")) {
            ...
            if (!range.toString()) {
                if (event.key === "ArrowRight" && isEndOfBlock(range) && !isIncludesHotKey("鈱モ嚙鈫?)) {
                    event.preventDefault();
                    event.stopPropagation();
                    return;
                }
                ...
                const position = getSelectionOffset(nodeEditableElement, protyle.wysiwyg.element, range);
                if (position.start === 0 && event.key === "ArrowLeft" && !isIncludesHotKey("鈱モ嚙鈫?)) {
                    event.preventDefault();
                    event.stopPropagation();
                    return;
                }
