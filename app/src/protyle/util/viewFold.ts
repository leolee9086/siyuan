...
import {getViewFoldVisualEffects} from "./imports";
import {
...
const renderHeadingChildren = (protyle: IProtyle, heading: Element, children: Element[]) => {
    ...
    const visualEffects = getViewFoldVisualEffects();
    visualEffects.renderHeadingChildren(protyle, children);
    if (protyle.disabled) {
        visualEffects.applyDisabledState(protyle);
    }
};
