...
import {genCellValue} from "../render/av/cell.value";
...
import {getTableRangeCells} from "./table/grid";
        ...
        for (let i = 0; i < options.values.length; i++) {
            for (let j = 0; j < options.values[i].length && j < targetColumns.length; j++) {
                ...
                const operations = await getAttrViewCellUpdate()({
                    ...
                    html: options.cellHTML?.[i]?.[j] || options.html,
                    getOperations: true,
                    forceOperation: isNewRow || targetColumn.isNew || targetColumn.typeChanged,
                    replaceSelectValues: true,
...
const processAV = (range: Range, html: string, protyle: IProtyle, blockElement: HTMLElement) => {
    ...
    fetchPost("/api/av/getAttributeViewKeysByAvID", {avID}, async (response) => {
        ...
        if (selectedRowCells.length > 0 && textJSON.length === 1 && textJSON[0].length === 1) {
            void getAttrViewCellUpdate()({
                protyle,
                nodeElement: blockElement as HTMLElement,
        ...
        if (cellElements.length > 0) {
            if (textJSON.length === 1 && textJSON[0].length === 1) {
                void getAttrViewCellUpdate()({
                    ...
                    nodeElement: blockElement as HTMLElement,
                    value: normalizedText,
                    cellElements: selectedCells.length > 0 ? undefined : cellElements,
                    columns,
                    html,
                });
            } else {
