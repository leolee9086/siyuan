// S-forge: 保持本地模块化导入与函数拆分架构，同时并入上游的资产上传任务/回调管线
import { insertHTML } from "../util/insertHTML";
import { hideMessage, showMessage, confirmDialog } from "../runtime/dialog.port";
...
import {getAttrViewCellUpdate} from "../render/av/cell/update/port";
...
import {setUploadRuntimeEffects} from "./runtime/port";
import type {IUploadInsertOptions} from "./runtime/types";
export type {IUploadInsertOptions} from "./runtime/types";
...
const handleUploadResult = (protyle: IProtyle, response: any) => {
    ...
    if (response.data.errFiles?.length > 0) {
        ...
        for (const data of response.data.errFiles) {
            ...
            errorTip += `<li>${escapeHtml(filename)} ${siyuanI18n.uploadError}</li>`;
        }
        errorTip += "</ul>";
    }
    if (errorTip) {
...
export const applyUploadedFiles = async (responseText: string, protyle: IProtyle, options?: IUploadInsertOptions) => {
    const response = JSON.parse(responseText);
    // 与上游一致：先提示失败文件，再继续插入成功文件（errFiles 与 succMap 可同时存在）
    handleUploadResult(protyle, response);
    ...
    if (document.querySelector(".av__panel")) {
        ...
        if (stableCells.length === 1 || cellElements.length === 1) {
            ...
            for (let i = 0; i < avAssets.length; i++) {
                ...
                const operations = await getAttrViewCellUpdate()({
                    protyle,
                    nodeElement,
                    value: [avAssets[i]],
                    cellElements: cellElement ? [cellElement] : undefined,
                    getOperations: true,
                    forceOperation: false,
                    replaceSelectValues: false,
                    stableCells: selectedCell ? [selectedCell] : undefined,
                });
...
const uploadPreparedLocalFiles = (input: Extract<IAssetUploadInput, { kind: "local-files" }>,
    ...
    input.files.forEach(item => {
        if (item.size && Constants.SIZE_UPLOAD_TIP_SIZE <= item.size) {
            msg += siyuanI18n.uploadFileTooLarge.replace("${x}", escapeHtml(item.path)).replace("${y}", filesize(item.size, {standard: "iec"})) + "<br>";
        }
    ...
    confirmDialog(msg ? siyuanI18n.upload : "", msg, () => {
        void (async () => {
            ...
            try {
                ...
                msgId = showMessage(siyuanI18n.uploading, 0);
                ...
                if (tip) {
                    showMessage(siyuanI18n.dndFolderTip.replace("${x}",
...
const uploadPreparedFiles = (input: Extract<IAssetUploadInput, { kind: "files" }>, protyle: IProtyle,
    ...
    for (let i = 0, iMax = validateResult.files.length; i < iMax; i++) {
        ...
        if (Constants.SIZE_UPLOAD_TIP_SIZE <= validateResult.files[i].size) {
            msg += siyuanI18n.uploadFileTooLarge.replace("${x}", escapeHtml(validateResult.files[i].name)).replace("${y}", filesize(validateResult.files[i].size, {standard: "iec"})) + "<br>";
        }
    }
    if (protyle.lite) {
...
/**
 * 上传本地文件
 * @param files 文件列表，支持资源路径字符串或本地文件描述对象
 * @param protyle 编辑器实例
 * @param isUpload 是否上传
 */
export const uploadLocalFiles = (files: string[] | ILocalFiles[], protyle: IProtyle, isUpload: boolean,
    ...
    // 兼容旧调用：纯字符串视为资源路径
    const localFiles: ILocalFiles[] = files.map(item => typeof item === "string" ? ({path: item, size: null}) : item);
    ...
    const callbacks: IAssetUploadCallbacks = {
        success(responseText, response, _input, result) {
            if (successCB) {
                ...
                return applyUploadedFiles(responseText, protyle, uploadOptions);
            }
        },
        complete: completeCB,
    };
    const input: IAssetUploadInput = {kind: "local-files", files: Array.from(localFiles)};
    if (!isUpload) {
...
/**
 * 作用：发布上传组合根的文件和本地路径命令。
 * 意图：低层编辑模块通过运行时端口调用完整任务管线，保持领域依赖单向。
 * 调用时机：本模块完成公开上传命令初始化后。
 * @同步豁免: 生命周期 - 首个上传事件前必须同步完成端口注册。
 */
setUploadRuntimeEffects({uploadFiles, uploadLocalFiles});
