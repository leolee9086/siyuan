...
import {turnsIntoOneTransaction} from "../../wysiwyg/transaction";
/** 用途：批量类型转换；使用范围：TurnInto 菜单动作；解耦评估：引用 transaction 领域唯一实现。 */
import {turnsIntoTransaction} from "../../wysiwyg/transaction";
/** 用途：单块类型转换；使用范围：TurnInto 菜单动作；解耦评估：引用 transaction 领域唯一实现。 */
import {turnsOneInto} from "../../wysiwyg/transaction";
