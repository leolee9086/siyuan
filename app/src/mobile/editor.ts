...
import type {IDatabaseItemOpenData, IDatabaseItemOpenOptions} from "../protyle/render/av/openDatabaseItem.types";
...
export const loadMobileFileById = (app: AppFacade, id: string, action: TProtyleAction[] = [Constants.CB_GET_HL],
...
export const openMobileFileByIdInNewTab = (app: AppFacade, id: string,
...
/** 移动宿主的数据库条目导航实现，保持编辑器完成后的定位激活。 */
const openDatabaseItemOnMobile = (app: AppFacade, data: IDatabaseItemOpenData, _options?: IDatabaseItemOpenOptions) => {
    const action = [Constants.CB_GET_CONTEXT, Constants.CB_GET_ROOTSCROLL] as TProtyleAction[];
    openMobileFileById(app, data.databaseBlockID, action, undefined, data.notebookID, (protyle) => {
        activateQueuedAVLocate({renderAV: getAVLocateRenderer(), protyle, blockID: data.databaseBlockID});
    });
    return true;
};

setDatabaseItemNavigator(openDatabaseItemOnMobile);
