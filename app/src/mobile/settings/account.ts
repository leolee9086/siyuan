                ...
                while (target && target !== modelMainElement) {
                    if (target.getAttribute("data-action") === "iOSPay") {
                        ...
                        fetchPost("/api/setting/logoutCloudUser", {}, () => {
                            ...
                            closePanel();
                            document.getElementById("menuAccount").innerHTML = `<svg class="b3-menu__icon"><use xlink:href="#iconAccount"></use></svg><span class="b3-menu__label">${siyuanI18n.login}</span>`;
                            processSync();
                        });
                        event.preventDefault();
                        event.stopPropagation();
                        ...
                        const dialog = new Dialog({
                            title: "⚠️ " + siyuanI18n.deactivateUser,
                            width: "92vw",
                        ...
                        fetchPost("/api/setting/getCloudUser", {
                            ...
                            if (window.siyuan.user) {
                                ...
                                menuAccountElement.innerHTML = `<svg class="b3-menu__icon"><use xlink:href="#iconAccount"></use></svg><span class="b3-menu__label">${siyuanI18n.login}</span>`;
                            }
                            processSync();
                        });
                        event.preventDefault();
...
<div class="fn__hr--b"></div>
<button id="login" class="b3-button fn__block">${siyuanI18n.deactivateUser}</button>`;
    } else {
...
<div class="b3-form__space fn__none" id="form2">
    ...
    <div class="fn__hr--b"></div>
    <button id="login2" class="b3-button fn__block">${deactivate ? siyuanI18n.deactivateUser : siyuanI18n.login}</button>
</div>`;
};
...
const bindLoginEvent = (modelMainElement: HTMLElement, deactive = false) => {
    ...
    if (cloudRegionElement) {
        cloudRegionElement.addEventListener("change", () => {
            ...
            modelMainElement.querySelector("#form1").lastElementChild.innerHTML = `<a href="${getCloudURL("forget-pwd")}" class="b3-button b3-button--cancel" target="_blank">${siyuanI18n.forgetPassword}</a>
        <span class="fn__space${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}"></span>
        <a href="${getCloudURL("register")}" class="b3-button b3-button--cancel${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}" target="_blank">${siyuanI18n.register}</a>`;
        });
    }
