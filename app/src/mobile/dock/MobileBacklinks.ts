...
export class MobileBacklinks {
    ...
    // 上游将面板根元素改为构造参数注入；分叉沿用 AppFacade 类型，并为旧调用点保留自查询回退
    constructor(app: AppFacade, element?: HTMLElement) {
        this.element = element ?? document.querySelector('#sidebar [data-type="sidebar-backlink"]');
