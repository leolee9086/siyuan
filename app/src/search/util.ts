...
import {getDisplayName, getNotebookIcon, getNotebookName, originalPath, pathPosix, useShell} from "../util/file/pathName";
...
// closeCB 不存在为页签搜索
export const genSearch = (app: AppFacade, config: Config.IUILayoutTabSearchConfig, element: HTMLElement, closeCB?: () => void) => {
...
<div class="fn__loading"><img width="120px" src="/stage/loading-pure.svg"></div>`;
    ...
    const edit = app.createProtyle(element.querySelector("#searchPreview") as HTMLElement, {
        blockId: "",
        render: {
            background: true,
            gutter: true,
    ...
    edit.resize();
    const unRefEdit = app.createProtyle(element.querySelector("#searchUnRefPreview") as HTMLElement, {
        blockId: "",
        render: {
            gutter: true,
            breadcrumbDocName: true,
        ...
        while (target && target !== element) {
            ...
            if (type === "removeCriterion") {
                ...
                if (type === "search-new") {
                    ...
                    if (isBrowser && isIPad()) { // 浏览器端 iPad 的 click detail 不可靠，需用时间差判断单双击 https://github.com/siyuan-note/siyuan/issues/12704
                    ...
                    if (isClick) {
                        ...
                        if (searchType === "asset") {
                            if (isElectron) { // 仅 Electron 端支持在系统文件管理器中定位资源文件
                                useShell("showItemInFolder", originalPath().join(window.siyuan.config.system.dataDir, target.lastElementChild.getAttribute("aria-label")));
                            }
                        } else {
                            openSearchEditor({
                                rootId: target.getAttribute("data-root-id"),
...
const emitBeforeSearchResultsRender = (blocks: IBlock[], edit: ProtyleDomain,
                                       config: Config.IUILayoutTabSearchConfig,
...
export const inputEvent = (element: Element, config: Config.IUILayoutTabSearchConfig,
                           edit: ProtyleDomain, rmCurrentCriteria = false,
                           focusId?: {
                               currentId?: string,
                               newId?: string
                           }) => {
...
const onSearch = (data: IBlock[], edit: ProtyleDomain, element: Element, config: Config.IUILayoutTabSearchConfig,
                  focusId?: {
                      currentId?: string,
                      newId?: string
                  }) => {
