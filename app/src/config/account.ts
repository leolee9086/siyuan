...
export const account = {
    ...
    <svg><use xlink:href="#iconVIP"></use></svg>${siyuanI18n.account4}
</button>`;
            } else {
                payHTML = `<button class="b3-button b3-button--big" data-action="iOSPay" data-type="subscribe">
    <svg><use xlink:href="#iconVIP"></use></svg>${siyuanI18n.account10}
</button>
<div class="fn__hr"></div>
<button class="b3-button b3-button--success" data-action="iOSPay" data-type="function">
    <svg><use xlink:href="#iconVIP"></use></svg>${siyuanI18n.onepay}
</button>`;
            }
        } else {
            payHTML = `<a class="b3-button b3-button--big" href="${getIndexURL("pricing.html")}" target="_blank">
    <svg><use xlink:href="#iconVIP"></use></svg>${siyuanI18n[window.siyuan.user?.userSiYuanOneTimePayStatus === 1 ? "account4" : "account1"]}
</a>`;
        }
...
<div>
${siyuanI18n.accountSupport1}
</div>
<div class="fn__hr--b"></div>
<div>
${siyuanI18n.accountSupport2}
</div>`;
        if (onlyPayHTML) {
...
<div class="fn__flex-1 fn__hr--b"></div>`;
   ...
   <input class="b3-text-field fn__block b3-form__icon-input" style="padding-right: 44px;" placeholder="${siyuanI18n.activationCodePlaceholder}">
   <button id="activationCode" class="b3-button b3-button--text" style="position: absolute;right: 0;top: 0;">${siyuanI18n.confirm}</button>
</div>`;
            if (window.siyuan.user.userSiYuanProExpireTime === -1) {
                // 终身会员
                activeSubscriptionHTML = "";
                subscriptionHTML = `<div class="b3-chip b3-chip--secondary">${Constants.SIYUAN_IMAGE_VIP}${siyuanI18n.account12}</div>`;
...
<div class="ft__on-surface ft__smaller">
    ...
    ${siyuanI18n.day} 
    ${isIOS ? `<a href="javascript:void(0)" data-action="iOSPay" data-type="subscribe">${siyuanI18n.clickMeToRenew}</a>` : `<a href="${getCloudURL("subscribe/siyuan")}" target="_blank">${siyuanI18n.clickMeToRenew}</a>`}
</div>`;
                if (window.siyuan.user.userSiYuanOneTimePayStatus === 1) {
                    subscriptionHTML = `<div class="b3-chip b3-chip--success"><svg><use xlink:href="#iconVIP"></use></svg>${siyuanI18n.account7}</div>
<div class="fn__hr--b"></div>`;
                }
                if (window.siyuan.user.userSiYuanSubscriptionPlan === 2) {
                    // 订阅试用
                    subscriptionHTML += `<div class="b3-chip b3-chip--primary"><svg><use xlink:href="#iconVIP"></use></svg>${siyuanI18n.account3}</div>
${renewHTML}<div class="fn__hr--b"></div>`;
                } else {
                    // 年费
                    subscriptionHTML += `<div class="b3-chip b3-chip--primary"><svg class="ft__secondary"><use xlink:href="#iconVIP"></use></svg>${siyuanI18n.account8}</div>
${renewHTML}<div class="fn__hr--b"></div>`;
                }
...
<div class="config-account__center">
    ...
    <div class="config-account__info">
        <div class="fn__flex">
            <a class="b3-button b3-button--text${isIOS ? " fn__none" : ""}" href="${getCloudURL("settings")}" target="_blank">${siyuanI18n.manage}</a>
            ...
            <button class="b3-button b3-button--cancel${window.siyuan.config.system.container === "ios" ? "" : " fn__none"}" id="deactivateUser">
                ${siyuanI18n.deactivateUser}
            </button>
            <span class="fn__flex-1"></span>
            <button class="b3-button b3-button--cancel b3-tooltips b3-tooltips__n" id="refresh" aria-label="${siyuanI18n.refresh}">
...
<div class="b3-form__space config-account__center">
    <div class="config-account__form" id="form1">
        <div class="b3-form__icon">
            ...
            <input id="userName" class="b3-text-field fn__block b3-form__icon-input" placeholder="${siyuanI18n.accountName}">
        </div>
        <div class="fn__hr--b"></div>
        <div class="b3-form__icon">
            <svg class="b3-form__icon-icon"><use xlink:href="#iconLock"></use></svg>
            <input type="password" id="userPassword" class="b3-text-field b3-form__icon-input fn__block" placeholder="${siyuanI18n.password}">
        </div>
        <div class="fn__hr--b"></div>
        <div class="b3-form__icon">
            ...
            <select class="b3-select b3-form__icon-input fn__block" id="cloudRegion">
                <option value="0"${window.siyuan.config.cloudRegion === 0 ? " selected" : ""}>${siyuanI18n.cloudRegionChina}</option>
                <option value="1"${window.siyuan.config.cloudRegion === 1 ? " selected" : ""}>${siyuanI18n.cloudRegionNorthAmerica}</option>
            </select>
        </div>
        <div class="b3-form__img fn__none">
            <div class="fn__hr--b"></div>
            <img id="captchaImg" class="fn__pointer" style="top: 17px;height:26px">
            <input id="captcha" class="b3-text-field fn__block" placeholder="${siyuanI18n.captcha}">
        </div>
        <div class="fn__hr--b"></div>
        <label class="ft__smaller ft__on-surface fn__flex">
            ...
            <span class="fn__space"></span>
            <span>${siyuanI18n.accountTip}</span>
        </label>
        <div class="fn__hr--b"></div>
        <button id="login" disabled class="b3-button fn__block">${siyuanI18n.login}</button>
        <div class="fn__hr--b"></div>
        <div class="ft__center">
            <a href="${getCloudURL("forget-pwd")}" class="b3-button b3-button--cancel" target="_blank">${siyuanI18n.forgetPassword}</a>
            <span class="fn__space${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}"></span>
            <a href="${getCloudURL("register")}" class="b3-button b3-button--cancel${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}" target="_blank">${siyuanI18n.register}</a>
        </div>
    </div>
    <div class="fn__none config-account__form" id="form2">
        <div class="b3-form__icon">
            <svg class="b3-form__icon-icon"><use xlink:href="#iconLock"></use></svg>
            <input id="twofactorAuthCode" class="b3-text-field fn__block b3-form__icon-input" placeholder="${siyuanI18n.twoFactorCaptcha}">
        </div>
        <div class="fn__hr--b"></div>
        <button id="login2" class="b3-button fn__block">${siyuanI18n.login}</button>
    </div>
</div>
<div class="config-account__center config-account__center--text${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}">
    ...
    bindEvent: (element: Element) => {
        ...
        if (!userNameElement) {
            ...
            refreshElement.addEventListener("click", () => {
                ...
                fetchPost("/api/setting/getCloudUser", {
                    ...
                    showMessage(siyuanI18n.refreshUser, 3000);
            ...
            element.querySelector("#deactivateUser").addEventListener(getEventName(), () => {
                confirmDialog("⚠️ " + siyuanI18n.deactivateUser, siyuanI18n.deactivateUserTip, () => {
            ...
            activationCodeElement?.addEventListener("click", () => {
                ...
                fetchPost("/api/account/checkActivationcode", {data: activationCodeInput.value}, (response) => {
                    ...
                    confirmDialog(siyuanI18n.activationCode, response.msg, () => {
                        if (response.code === 0) {
        ...
        cloudRegionElement.addEventListener("change", () => {
            ...
            element.querySelector("#form1").lastElementChild.innerHTML = `<a href="${getCloudURL("forget-pwd")}" class="b3-button b3-button--cancel" target="_blank">${siyuanI18n.forgetPassword}</a>
<span class="fn__space${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}"></span>
<a href="${getCloudURL("register")}" class="b3-button b3-button--cancel${window.siyuan.config.system.container === "ios" ? " fn__none" : ""}" target="_blank">${siyuanI18n.register}</a>`;
        });
        ...
        if (window.siyuan.config.account.displayVIP) {
            if (window.siyuan.user) {
                if (window.siyuan.user.userSiYuanProExpireTime === -1) { // 终身会员
                    html = `<div class="toolbar__item ariaLabel" aria-label="${siyuanI18n.account12}">${Constants.SIYUAN_IMAGE_VIP}</div>`;
                } else if (window.siyuan.user.userSiYuanProExpireTime > 0) { // 订阅中
                    if (window.siyuan.user.userSiYuanSubscriptionPlan === 2) { // 试用订阅
                        html = `<div class="toolbar__item ariaLabel" aria-label="${siyuanI18n.account3}"><svg><use xlink:href="#iconVIP"></use></svg></div>`;
                    } else { // 正常订阅
                        html = `<div class="toolbar__item ariaLabel" aria-label="${siyuanI18n.account10}"><svg class="ft__secondary"><use xlink:href="#iconVIP"></use></svg></div>`;
                    }
                } else if (window.siyuan.user.userSiYuanSubscriptionStatus === -1) { // 未订阅
                    html = `<div class="toolbar__item ariaLabel" aria-label="${siyuanI18n.freeSub}"><svg class="ft__error"><use xlink:href="#iconVIP"></use></svg></div>`;
                }
                if (window.siyuan.user.userSiYuanOneTimePayStatus === 1) { // 一次性付费功能特性
                    html += `<div class="toolbar__item ariaLabel" aria-label="${siyuanI18n.onepay}"><svg class="ft__success"><use xlink:href="#iconVIP"></use></svg></div>`;
                }
            } else { // 未登录
                html = `<div class="toolbar__item ariaLabel" aria-label="${siyuanI18n.freeSub}"><svg class="ft__error"><use xlink:href="#iconVIP"></use></svg></div>`;
            }
