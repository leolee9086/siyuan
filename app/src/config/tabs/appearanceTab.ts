...
import {isInMobileApp, updateHotkeyTip} from "../../protyle/util/compatibility";
        ...
        afterMount: (root) => {
            if (isElectron) {
                root.querySelector("#appearanceOpenTheme")?.addEventListener("click", () => {
                    useShell("openPath", originalPath().join(window.siyuan.config.system.confDir, "appearance", "themes"));
                });
            }
        },
    }, (stack) => {
        ...
        afterMount: (root) => {
            if (isElectron) {
                root.querySelector("#appearanceOpenIcon")?.addEventListener("click", () => {
                    useShell("openPath", originalPath().join(window.siyuan.config.system.confDir, "appearance", "icons"));
                });
            }
        },
    }, (stack) => {
        ...
        group.button({
            ...
            afterMount: (root) => {
                root.querySelector("#appearanceOpenEmoji")?.addEventListener("click", () => {
                    useShell("openPath", originalPath().join(window.siyuan.config.system.dataDir, "emojis"));
                });
            },
        });
    }
