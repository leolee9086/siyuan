 ...
 */
  ...
  /**
 * @typedef {import('./index.d.ts').ImportImportZipMdResponse} ImportImportZipMdResponse
 * 导入包含 Markdown 文件和资源的 zip 包到指定的笔记本和路径下。请求体为 FormData。必须包含 'file' 字段 (值为 .zip 文件), 'notebook' 字段 (目标笔记本ID), 'toPath' 字段 (导入到笔记本内的目标父路径，例如 '/' 表示笔记本根目录)。由于是 FormData，具体字段不在 Zod schema 中定义。导入过程会将文件暂存、解压并在处理后删除。后台会显示进度。
 * (Requires authentication, Requires admin role, Unavailable in read-only mode)
 * @returns {Promise<ImportImportZipMdResponse>}
 */
  importZipMd() {
    return this.fetcher('POST', '/api/import/importZipMd', {}, true);
  }
