...
export function createHNSWIndex(config: HNSWConfig): HNSWIndex {
  ...
  function searchLayerWithQuery(
    ...
    while (candidates.length > 0) {
      ...
      const neighborsLength = nodeNeighbors.length;
  ...
  function connectNeighbors(nodeIdx: number, level: number, nearestNeighbors: Neighbor[]) {
    ...
    const selectedNeighborsLength = selectedNeighbors.length;
    for (let i = 0; i < selectedNeighborsLength; i++) {
        const neighbor = selectedNeighbors[i]!; // @织: BUG修复 - 只连接被选中的邻居
